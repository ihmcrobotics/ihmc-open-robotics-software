...
public class NewExitCMPCurrentMultiplierTest
{
   ...
   public void testCalculationRecurseBackwardTwoCMPFirstSegment()
   {
      ...
      for (int iter = 0; iter < iters; iter++)
      {
         ...
         Assert.assertEquals("iter = " + iter, 1.0 - projection, exitCMPCurrentMultiplier.getPositionMultiplier(), epsilon);
         Assert.assertEquals("iter = " + iter, -omega * projection, exitCMPCurrentMultiplier.getVelocityMultiplier(), epsilon);
      }
   }

...
public class NewInstantaneousCapturePointPlannerTest
{
   ...
   public void visualizePlanner()
   {
      ...
      ArrayList<FramePoint> footLocations = createABunchOfUniformWalkingSteps(stepSide, startSquaredUp, numberOfStepsInStepList, stepLength, halfStepWidth);
      ...
      while (footLocations.size() >= 2)
      {
         ...
         simulateForwardAndCheckSingleSupport(icpPosition, icpVelocity, icpAcceleration, cmpPosition, icpPlanner, singleSupportDuration, initialTime, omega0,
               initialICPPosition, footLocations);
   ...
   private ArrayList<FramePoint> createABunchOfUniformWalkingSteps(RobotSide stepSide, boolean startSquaredUp, int numberOfStepsInStepList, double stepLength,
   ...
   private void simulateForwardAndCheckSingleSupport(FramePoint icpPositionToPack, FrameVector icpVelocityToPack, FrameVector icpAccelerationToPack,
         FramePoint ecmpPositionToPack, NewInstantaneousCapturePointPlanner icpPlanner, double singleSupportDuration, double initialTime, double omega0,
         FramePoint initialICPPosition, ArrayList<FramePoint> footstepList)
   {
      for (double time = initialTime + deltaT; time <= initialTime + singleSupportDuration; time = time + deltaT)
      {
         ...
         if (visualize)
         {
            visualizeICPAndECMP(icpPositionToPack, icpVelocityToPack, icpPlanner.getConstantCentroidalMomentumPivots().get(0).getFramePointCopy(), time);
         }
   ...
   private void simulateForwardAndCheckDoubleSupport(ArrayList<FramePoint> footstepList, FramePoint icpPositionToPack, FrameVector icpVelocityToPack,
         FrameVector icpAccelerationToPack, FramePoint ecmpPositionToPack, NewInstantaneousCapturePointPlanner icpPlanner, double doubleSupportDuration,

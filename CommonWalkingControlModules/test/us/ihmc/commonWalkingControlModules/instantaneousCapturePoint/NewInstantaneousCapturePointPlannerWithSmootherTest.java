...
public class NewInstantaneousCapturePointPlannerWithSmootherTest
{
   ...
   public void testCapturePointPlannerWithSmootherNoPushOrCancelPlan()
   {
      ...
      simulateForwardAndCheckSingleSupport(icpPosition, icpVelocity, icpAcceleration, cmpPosition, icpPlanner, singleSupportDuration, initialTime, omega0,
            initialICPPosition, footLocations, false);
      ...
      expectedICP.setX(constantCMPs.get(0).getX() * (1 - Math.exp(omega0 * singleSupportDuration))
            + cornerPoints.get(0).getX() * Math.exp(omega0 * singleSupportDuration));
      expectedICP.setY(constantCMPs.get(0).getY() * (1 - Math.exp(omega0 * singleSupportDuration))
            + cornerPoints.get(0).getY() * Math.exp(omega0 * singleSupportDuration));
   ...
   private void simulateForwardAndCheckSingleSupport(FramePoint icpPositionToPack, FrameVector icpVelocityToPack, FrameVector icpAccelerationToPack,
         FramePoint cmpPositionToPack, NewInstantaneousCapturePointPlannerWithSmoother icpPlanner, double singleSupportDuration, double initialTime,
         double omega0, FramePoint initialICPPosition, FrameTupleArrayList<FramePoint> footstepList, boolean cancelPlan)
   {

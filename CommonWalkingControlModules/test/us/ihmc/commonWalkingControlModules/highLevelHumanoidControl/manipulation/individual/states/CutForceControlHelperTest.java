...
public class CutForceControlHelperTest
{
   ...
   public void testModelParameterAdaption()
   {
      ...
      while (Math.abs(CutForceControlHelper.exponentialForceModel(v, C1adapted.getDoubleValue(), C2adapted.getDoubleValue())
            - CutForceControlHelper.exponentialForceModel(v, C1, C2)) > VELOCITYERROR)
      {
   ...
   public void testGetTangentForce()
   {
      ...
      CutForceControlHelper.getTangentForce(forceVector, currentTangentialForce, tangentVector, lastTangentVector, fxFiltered, fyFiltered, fzFiltered, false,
            0.0);

      assertEquals(forceVector.dot(tangentVector), currentTangentialForce.getDoubleValue(), EPSILON);
      assertEquals(currentTangentialForce.getDoubleValue(),
            fxFiltered * tangentVector.getX() + fyFiltered * tangentVector.getY() + fzFiltered * tangentVector.getZ(), EPSILON);
   ...
   public void testWristSensorUpdate()
   {
      ...
      CutForceControlHelper.wristSensorUpdate(testWrench, worldFrame, fxRaw, fyRaw, fzRaw, fxFiltered, fyFiltered, fzFiltered, mass, false,
            randomNumberGenerator, 0.0);

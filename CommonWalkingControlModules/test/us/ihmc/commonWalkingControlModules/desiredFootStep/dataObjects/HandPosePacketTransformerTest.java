...
{
	private static final int NUMBER_OF_ARM_JOINTS = 6;
   ...
   public void testTransformHandPosePacket()
   {
      ...
      for (int i = 0; i < numberOfTests; i++)
      {
         ...
         double[] jointAngles = new double[NUMBER_OF_ARM_JOINTS];
         for(int j = 0; j < NUMBER_OF_ARM_JOINTS; j++)
         {
        	 jointAngles[j] = RandomTools.generateRandomDouble(random, -2 * Math.PI, 2 * Math.PI);
         }

         HandPosePacket starting = new HandPosePacket(robotSide, frame, point3d, quat, i % 2 == 1, trajectoryTime, jointAngles);

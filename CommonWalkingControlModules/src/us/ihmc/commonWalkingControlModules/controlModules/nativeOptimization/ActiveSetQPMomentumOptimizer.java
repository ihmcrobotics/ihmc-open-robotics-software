...
public class ActiveSetQPMomentumOptimizer extends QPMomentumOptimizer implements MomentumOptimizerInterface, Serializable
{
   ...
   public int solve(boolean clearActiveSet) throws NoConvergenceException
   {
      ...
      if (useJNA)
      {
         iter = JnaInterface.INSTANCE.solveNative(A.getData(), b.getData(), C.getData(), Jp.getData(), pp.getData(), Js.getData(), ps.getData(), Ws.getData(),
               WRho.getData(), Lambda.getData(), WRhoSmoother.getData(), rhoPrevMean.getData(), WRhoCoPPenalty.getData(), QRho.getData(), c.getData(),
               rhoMin.getData(), vd.getData(), rho.getData(), // in/out
               activeSet);
      }
      else
      {
         iter = solveNative(A.getData(), b.getData(), C.getData(), Jp.getData(), pp.getData(), Js.getData(), ps.getData(), Ws.getData(), WRho.getData(),
               Lambda.getData(), WRhoSmoother.getData(), rhoPrevMean.getData(), WRhoCoPPenalty.getData(), QRho.getData(), c.getData(), rhoMin.getData(),
   ...
   private native int solveNative(double[] A, double[] b, double[] C, double[] Jp, double[] pp, double[] Js, double[] ps, double[] Ws, double[] WRho,
         double[] Lambda, double[] WRhoSmoother, double[] rhoPrevMean, double[] WRhoCoPPenalty, double[] QRho, double[] c, double[] rhoMin, double[] vd,
         double[] rho, int[] activeSet);

   public interface JnaInterface extends Library
   {
      int solveNative(double[] A, double[] b, double[] C, double[] Jp, double[] pp, double[] Js, double[] ps, double[] Ws, double[] WRho, double[] Lambda,
            double[] WRhoSmoother, double[] rhoPrevMean, double[] WRhoCoPPenalty, double[] QRho, double[] c, double[] rhoMin, double[] vd, double[] rho,
            int[] activeSet);
   ...
   public static void main(String[] arg) throws IOException, ClassNotFoundException, NoConvergenceException, IllegalArgumentException, IllegalAccessException,

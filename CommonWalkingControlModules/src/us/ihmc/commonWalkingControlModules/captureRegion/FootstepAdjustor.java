...
//      footstep.setPositionChangeOnlyXY(nextStep2d);
   ...
   private final FramePoint2d pointToMove = new FramePoint2d();
   /**
   ...
   private void calculateTouchdownFootPolygon(Footstep footstep, ReferenceFrame desiredFrame, FrameConvexPolygon2d polygonToPack)
   {
      ...
      polygonToPack.clear(desiredFrame);
      ...
      for(int i = 0; i < expectedContactPoints.size(); i++)
      {
         ...
         pointToMove.sub(centroid);
         pointToMove.scale(SHRINK_TOUCHDOWN_POLYGON_FACTOR);
         pointToMove.add(centroid);
         
         polygonToPack.addVertexAndChangeFrame(pointToMove);
      }
    ...
    */
   private void updateDesiredSteppingRegion(FrameConvexPolygon2d captureRegion, SteppingStones steppingStones,

...
public abstract class CapturePointPlannerParameters
{
    ...
    * the actual ICP is behind the plan, and totally freeze time if the actual ICP is far behing the
   ...
   /**
    * Only used when using the new ICP planner with two CMPs per support. This parameter forces the
    * exit CMPs to be more inside or outside in the foot.
    */
   ...
   /**
    * Only used when using the new ICP planner with two CMPs per support. This parameter forces the
    * exit CMPs to be more forward or backward in the foot.
    */
   public abstract double getExitCMPForwardOffset();

   /**
    * Only used when using the new ICP planner. Refers to whether use one or two CMPs reference
    * locations per support. Using two CMPs is preferable as it helps the robot moving more towards
    * the walking heading direction. It also helps triggering the toe off earlier, and helps
    * stepping down obstacles. However, it requires to tune some more parameters.
    */
    ...
    * exit CMP.
    * <p>
    * Only used when {@link #useTwoCMPsPerSupport()} is {@code true}.
    * </p>
    * <p>
    * The output the ICP planner is quite sensitive to this parameter and the plan
    * quality/feasibility can be greatly reduced with bad a tuning.
   ...
   /**
    * This parameter indicates how far front in the foot the entry CMP can be. A large positive
    * value here can be helpful for back steps.
    */
   public abstract double getMaxEntryCMPForwardOffset();

   /**
    * This parameter indicates how far back in the foot the exit CMP can be. Should probably be 0.0,
    * a negative value would delay the toe off when walking forward.
    */
   public abstract double getMinEntryCMPForwardOffset();

   /**
    * This parameter indicates how far front in the foot the exit CMP can be. A large positive value
    * here can improve significantly long forward steps and help trigger the toe off earlier.
    */
   public abstract double getMaxExitCMPForwardOffset();

   /**
    * This parameter indicates how far back in the foot the exit CMP can be. For instance, -0.02m
    * will let the robot move slightly backward in single support when doing back steps.
    */
   public abstract double getMinExitCMPForwardOffset();

   /**
    * This parameter is used when computing the entry CMP and exit CMP to make sure they are
   ...
   /**
    * Pretty much refers to how fast the CMP should move from the entry CMP to the exit CMP in
   ...
   /**
    * Only used when using the new ICP planner with two CMPs per support. The forward offset of the
    * CMPs is computed according to the upcoming step length times the returned factor One third
   ...
   /**
    * Only used when using the new ICP planner with two CMPs per support. If true, the ICP planner
    * will put the exit CMP on the toes of the trailing foot when stepping down and forward.
    */
   ...
   /**
    * Only used when using the new ICP planner with two CMPs per support. Threshold used to figure
    * out if the exit CMP should be put on the toes when stepping down.
    */
   ...
   /**
    * Only used when using the new ICP planner with two CMPs per support. Threshold used to figure
    * out if the exit CMP should be put on the toes. An absolute value is expected.
    */
   ...
   /**
    * Only used when using the new ICP planner with two CMPs per support. If set to zero, the exit
    * CMP will be on the toes' edge when stepping, a positive value will pull back the exit CMP
    * towards the foot center.
    */
   ...
   /**
    * Only used when using the new ICP planner with two CMPs per support. Threshold used to figure
    * out if the exit CMP should be put on the toes.
    */

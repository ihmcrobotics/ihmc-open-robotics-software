   ...
   public JointPDHighLevelHumanoidController(MomentumBasedController momentumBasedController, double initialKpGains, double initialKdGains)
   {
      ...
      for (int i = 0; i < fullRobotModel.getOneDoFJoints().length; i++)
      {
         ...
         if (joinName.contains("neck"))
            continue;
         if (joinName.contains("arm"))
            continue;

         jointsBeenControlled.add(joint);

         /*
          * DoubleYoVariable kp = new DoubleYoVariable("hl_" + joint.getName()+
          * CommonNames.k_q_p, registry); kp.set( initialKpGains );
          * kpJoints.put(joint, kp); DoubleYoVariable kd = new
          * DoubleYoVariable("hl_" + joint.getName() + CommonNames.k_qd_p,
          * registry); kd.set( initialKdGains ); kdJoints.put(joint, kd);
          */

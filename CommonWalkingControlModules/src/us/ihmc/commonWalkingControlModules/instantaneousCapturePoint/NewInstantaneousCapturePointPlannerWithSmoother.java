...
public class NewInstantaneousCapturePointPlannerWithSmoother
{
	...
	private final YoFramePoint singleSupportInitialDesiredCapturePointPosition = new YoFramePoint("icpFinalDesiredCapturePointPosition",
			worldFrame, registry);
	private final YoFrameVector singleSupportInitialDesiredCapturePointVelocity = new YoFrameVector("icpFinalDesiredCapturePointVelocity",
			worldFrame, registry);
	private final YoFramePoint desiredCentroidalMomentumPivotPosition = new YoFramePoint("icpPlannerDesiredCentroidalMomentumPosition",
			worldFrame, registry);
	...
	private final YoFrameVector desiredCapturePointVelocity = new YoFrameVector("icpPlannerDesiredCapturePointVelocity", worldFrame,
			registry);
			...
			double initialTime, ArrayList<FramePoint> footstepList)
	{
		...
		computeUpcomingSingleSupportInitialDesiredCapturePointPosition(capturePointCornerPoints.get(1),
				constantCentroidalMomentumPivots.get(1),
				doubleSupportSplitFraction.getDoubleValue() * doubleSupportDuration.getDoubleValue());
		...
		if (atAStop.getBooleanValue())
		{
			initializeDoubleSupportCapturePointTrajectory(desiredCapturePointPosition, desiredCapturePointVelocity,
					singleSupportInitialDesiredCapturePointPosition, singleSupportInitialDesiredCapturePointVelocity,
					doubleSupportInitialTransferDuration);
		}
		else
		{
			initializeDoubleSupportCapturePointTrajectory(desiredCapturePointPosition, desiredCapturePointVelocity,
	...
	public void initializeSingleSupport(double initialTime, ArrayList<FramePoint> footstepList)
	{
		...
		computeUpcomingSingleSupportInitialDesiredCapturePointPosition(capturePointCornerPoints.get(0),
				constantCentroidalMomentumPivots.get(0),
				doubleSupportSplitFraction.getDoubleValue() * doubleSupportDuration.getDoubleValue());
	}

	private void computeUpcomingSingleSupportInitialDesiredCapturePointPosition(YoFramePoint initialCapturePointPosition,
			YoFramePoint initialCenterOfPressurePosition, double time)
	{
		CapturePointTools.computeDesiredCapturePointPosition(this.omega0.getDoubleValue(), time, initialCapturePointPosition,
				initialCenterOfPressurePosition, singleSupportInitialDesiredCapturePointPosition);
	}

	private void computeInitialVelocityOfUpcomingSingleSupportPhase()
	{
		CapturePointTools.computeDesiredCapturePointVelocity(this.omega0.getDoubleValue(), 0.0,
				singleSupportInitialDesiredCapturePointPosition, constantCentroidalMomentumPivots.get(1),
				singleSupportInitialDesiredCapturePointVelocity);
	}
	...
	protected void computeConstantCentersOfPressure(ArrayList<FramePoint> footstepList)
	{
		...
		if (!wasPushedInSingleSupport.getBooleanValue())
		{
			if (atAStop.getBooleanValue())
			{
				if (!comeToStop.getBooleanValue())
				{
					CapturePointTools.computeConstantCentersOfPressureWithStartBetweenFeetAndRestOnFeet(constantCentroidalMomentumPivots,
							footstepList, numberOfCentersOfPressureToPlan);
				}
				else
				{
					CapturePointTools.computeConstantCentersOfPressuresWithBeginningAndEndBetweenFeetRestOnFeet(
							constantCentroidalMomentumPivots, footstepList, numberOfCentersOfPressureToPlan);
				}
			}
			else
			{
				if (comeToStop.getBooleanValue())
				{
					CapturePointTools.computeConstantCentersOfPressuresOnFeetWithEndBetweenFeet(constantCentroidalMomentumPivots,
							footstepList, numberOfCentersOfPressureToPlan);
				}
				else
				{
					CapturePointTools.computeConstantCentersOfPressuresOnFeet(constantCentroidalMomentumPivots, footstepList,
	...
	protected void computeCapturePointCornerPoints(double steppingDuration)
	{
		if (!wasPushedInSingleSupport.getBooleanValue())
		{
			CapturePointTools.computeDesiredEndOfStepCapturePointLocations(constantCentroidalMomentumPivots, capturePointCornerPoints,
					steppingDuration, omega0.getDoubleValue());
		}
		else
		{
			CapturePointTools.computeDesiredEndOfStepCapturePointLocationsWithFirstLeftUnset(constantCentroidalMomentumPivots,
					capturePointCornerPoints, this.doubleSupportDuration.getDoubleValue() + this.singleSupportDuration.getDoubleValue(),
	...
	public void updateForSingleSupportPush(ArrayList<FramePoint> footstepList, double time)
	{
		...
		CapturePointTools.computeConstantCenterOfPressureFromInitialAndFinalCapturePointLocations(capturePointCornerPoints.get(1),
				capturePointCornerPoints.get(0), constantCentroidalMomentumPivots.get(0), this.omega0.getDoubleValue(), timeRemaining
	...
	private void cancelPlanNow(double time, ArrayList<FramePoint> footstepList)
	{
		...
		CapturePointTools.computeConstantCenterOfPressureFromInitialAndFinalCapturePointLocations(
				singleSupportInitialDesiredCapturePointPosition, desiredCapturePointPosition, constantCentroidalMomentumPivots.get(0),
				omega0.getDoubleValue(), doubleSupportDuration.getDoubleValue());

...
public class CenterOfMassKinematicBasedLinearStateCalculator
{
   ...
   public void initialize(FramePoint comInitialPosition)
   {
      ...
      alphaFootToRootJointPosition.set(0.999);//XXX: tune me!!!
   ...
   public void estimateComPositionAndVelocity(ArrayList<RobotQuadrant> feetInContact, ArrayList<RobotQuadrant> feetNotInContact, QuadrantDependentList<YoFramePoint> estimatedYoFeetPositions)
   {
      updateKinematics();
      ...
      else
      {
         ...
         for (int i = 0; i < feetInContact.size(); i++)
         {
            ...
            estimatedYoFeetPositions.get(quadrantInContact).getFrameTuple(tempPosition);
            tempPosition.scale(scaleFactor);
            rootJointPosition.add(tempPosition);

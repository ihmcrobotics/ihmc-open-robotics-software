...
public class DRCPosePlaybackDemo
{
			...
			DRCSCSInitialSetup scsInitialSetup, PosePlaybackPacket posePlaybackPacket, DRCRobotModel model)
	{
		...
		controllerFactory = new MomentumBasedControllerFactory(contactableBodiesFactory, model.getSensorInformation().getFeetForceSensorNames(),
		      model.getSensorInformation().getFeetContactSensorNames(), walkingControllerParameters,
	...
	private class HoldRobotInTheAir implements RobotController
	{
		...
		public void doControl()
		{
			for (int i = 0; i < efp_offsetFromRootJoint.size(); i++)
			{
				...
				// proportionalTerm.setZ(Math.max(proportionalTerm.getZ(),
				// 0.0));

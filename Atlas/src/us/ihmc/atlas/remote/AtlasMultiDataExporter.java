   ...
   @Override
	...
	private class SimulateAndExport
	{
		   ...
		   private void exportDataAndVideo()
		   {
			   ...
			   /*
		       try {
			   exportM3Data.finalize();
				} catch (Throwable e) {
					e.printStackTrace();
				}
			   try 
		       {
				   exportM3Data.simulation.join();
				   exportM3Data.scs.closeAndDispose();
				   exportM3Data.scs.stopSimulationThread();
				   exportM3Data.simulation.stop();
			   } 
		       catch (Throwable e) 
			   {
		   ...
		   {
		      if (exportM3Data.players != null) exportM3Data.players.setActivePlayer(videoName);
		      
		   }
		
		   private void writeSpreadsheetFormattedData(File chosenFile, DataBuffer dataBuffer, ArrayList<? extends YoVariable> vars, 
				                                      String timeVariable)
		   {

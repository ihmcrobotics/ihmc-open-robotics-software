...
float3 estimate_perspective_normal(read_write image2d_t in, int rIndex,
                                   int cIndex, global float *params)
{
   float residual = 0;
   float radius = 0;
   ...
   if (rIndex >= m && cIndex >= m &&
       (rIndex * ((int)params[PATCH_HEIGHT]) + 2 * m) <
           ((int)params[INPUT_HEIGHT]) &&
       (cIndex * ((int)params[PATCH_WIDTH]) + 2 * m) <
...
float3 estimate_spherical_normal(read_write image2d_t in, int rIndex,
                                 int cIndex, global float *params)
{
   float residual = 0;
   float radius = 0;
...
float3 estimate_perspective_centroid(read_write image2d_t in, int y, int x,
                                     global float *params)
{
   float Z = 0;
   int count = 0;
...
float3 estimate_spherical_centroid(read_write image2d_t in, int rIndex,
                                   int cIndex, global float *params)
{
   float radius = 0;
   int count = 0;
   float3 centroid = (float3)(0, 0, 0);
   // if(rIndex >= 0 && rIndex < (int)params[SUB_H] && cIndex >= 0 && cIndex <
   // (int)params[SUB_W])
   {
      for (int i = 0; i < (int)params[PATCH_HEIGHT]; i++)
      {
                 ...
                 global float *params)
{
   ...
   if (perpDist < params[MERGE_ORTHOGONAL_THRESHOLD] &&
       (dist < distThreshold * params[MERGE_DISTANCE_THRESHOLD]) &&
       sim > params[MERGE_ANGULAR_THRESHOLD] && distanceToNearPoint > 0.8f)
   {
      return true;
   }
   else
 ...
 */
void fillDeadPixels(read_write image2d_t inputImage, int x, int y,
...
void smooth_non_boundary(read_write image2d_t in, int x, int y,
                         write_only image2d_t out0, global float *params)
{
   uint Z = 0;
   int m = 5;
 ...
 */
void kernel filterKernel(read_write image2d_t inputImage,
...
void kernel packKernel(
    read_write image2d_t in, write_only image2d_t out0,
...
)
{
   ...
   //    printf("[r:%d,c:%d] ->
   //    PackKernel:(%d,%d,%d,%d,%d,%d,%d,%.2lf,%.2lf)\n", rIndex, cIndex,
   //                            (int)params[INPUT_HEIGHT],
   //                            (int)params[INPUT_WIDTH],
   //                            (int)params[SUB_H],
   //                            (int)params[SUB_W],
   ...
   // if(cIndex >= 0 && cIndex < (int)params[SUB_H] && rIndex >= 0 && rIndex <
   // (int)params[SUB_W])
   {
      ...
      //      printf("PackKernel[%d,%d]\t Centroid:(%.4lf, %.4lf, %.4lf)\t
      //      Normal:(%.4lf,%.4lf,%.4lf)\n", rIndex, cIndex, centroid.x,
      //      centroid.y, centroid.z, normal.x, normal.y, normal.z);
   }
}
...
void kernel mergeKernel(
    read_write image2d_t in, read_only image2d_t out0, read_only image2d_t out1,
...
)
{
   ...
   if (rIndex >= m && rIndex < (int)params[SUB_H] - m && cIndex >= m &&
       cIndex < (int)params[SUB_W] - m)
   {
      ...
      write_imageui(out6, (int2)(cIndex, rIndex),
...
/*
 * Perspective Back-Projection Kernel for Structured Light And Perspective
 * Time-of-Flight Sensors
 * */
void kernel perspectiveBackProjectionKernel(read_write image2d_t in,
                                            global float *cloud,
                                            global float *params)
{
   int cIndex = get_global_id(0);
   ...
   // if(rIndex >= 0 && rIndex < (int)params[INPUT_HEIGHT] && cIndex >= 0 &&
   // cIndex < (int)params[INPUT_WIDTH])
   {
      ...
      if (radius > 0.3f)
      {
         ...
         // printf("[%d] Spherical(%d,%d):\t Radius: %.3lf, Point:(%.4lf, %.4lf,
         // %.4lf)\n", index, rIndex, cIndex, radius, cloud[index],
         // cloud[index+1], cloud[index+2]);
      }
   }
}
 ...
 * */
void kernel sphericalBackProjectionKernel(read_write image2d_t in,
                                          global float *cloud,
                                          global float *params)
{
   int cIndex = get_global_id(0);
 ...
 */
void kernel copyKernel(read_only image2d_t in0, read_only image2d_t in1,
                       read_only image2d_t in2, read_only image2d_t in3,
                       read_only image2d_t in4, read_only image2d_t in5,
                       write_only image2d_t out0, write_only image2d_t out1,
                       write_only image2d_t out2, write_only image2d_t out3,
                       write_only image2d_t out4, write_only image2d_t out5,
                       global float *params)
{
   int cIndex = get_global_id(0);
   int rIndex = get_global_id(1);
   ...
   //      printf("CopyKernel[%d,%d] -> (%.3lf, %.3lf, %.3lf) (%.3lf, %.3lf,
 ...
 */
void kernel
correspondenceKernel(read_only image2d_t one0, read_only image2d_t one1,
                     read_only image2d_t one2, read_only image2d_t one3,
                     read_only image2d_t one4, read_only image2d_t one5,
                     read_only image2d_t two0, read_only image2d_t two1,
                     read_only image2d_t two2, read_only image2d_t two3,
                     read_only image2d_t two4, read_only image2d_t two5,
                     write_only image2d_t matchRow,
                     write_only image2d_t matchColumn, global float *params)
{
   int cIndex = get_global_id(0);
   int rIndex = get_global_id(1);
   ...
   for (int i = 0; i < 4; i++)
   {
      for (int j = 0; j < 4; j++)
      {
         ...
         if (rowTwo >= 0 && rowTwo < totalRows && columnTwo >= 0 &&
             columnTwo < totalColumns)
         {
            ...
            // pointTwo = (float4)(cloudTwo[j*3+0], cloudTwo[j*3+1],
            // cloudTwo[j*3+2], 0); pointTwo = transform(pointTwo,
            // (float4)(transformTwo[0], transformTwo[1], transformTwo[2], 0),
   ...
   //        columnOne, minRowIndex, minColumnIndex, minLength);
   //    }

   write_imageui(matchRow, (int2)(cIndex, rIndex),
                 (uint4)(minRowIndex, 0, 0, 0));
   write_imageui(matchColumn, (int2)(cIndex, rIndex),
                 (uint4)(minColumnIndex, 0, 0, 0));
}

/*
 * Centroid Calculation Kernel for Iterative Closest Point
 * */
void kernel centroidReduceKernel(
    read_write image2d_t one0, read_write image2d_t one1,
    read_write image2d_t one2, read_write image2d_t one3,
    read_write image2d_t one4, read_write image2d_t one5,
    read_write image2d_t two0, read_write image2d_t two1,
    read_write image2d_t two2, read_write image2d_t two3,
    read_write image2d_t two4, read_write image2d_t two5,
    read_write image2d_t matchRowImage, read_write image2d_t matchColumnImage,
    global float *mean, global float *params)
{
   int cIndex = get_global_id(0);
   ...
   {
      //        if(rIndex == 0 && cIndex == 0) printf("Centroid Reduce
      //        Kernel\n");
   ...
   //   printf("Mean(%d) Count(%d): (%.3lf, %.3lf, %.3lf, %.3lf, %.3lf,
   //   %.3lf)\n", cIndex, count,
   //          mean[cIndex*6], mean[cIndex*6 + 1], mean[cIndex*6 + 2],
   //          mean[cIndex*6 + 3], mean[cIndex*6 + 4], mean[cIndex*6 + 5]);
}

/*
 * ICP Kernel for Iterative Closest Point
 * */
void kernel correlReduceKernel(
    read_write image2d_t one0, read_write image2d_t one1,
    read_write image2d_t one2, read_write image2d_t one3,
    read_write image2d_t one4, read_write image2d_t one5,
    read_write image2d_t two0, read_write image2d_t two1,
    ...
    read_write image2d_t two4, read_write image2d_t two5,
    read_write image2d_t matchRowImage, read_write image2d_t matchColumnImage,
    global float *correlation, global float *params)
{
   int cIndex = get_global_id(0);
   ...
   for (int rIndex = 0; rIndex < params[0]; rIndex++)
   {
      //        if(rIndex == 0 && cIndex == 0) printf("Correlation Reduce
      //        Kernel\n");
      ...
      if (matchRow != 0 && matchColumn != 0)
      {
         ...
         float cz1 = read_imagef(one5, pos).x;
         pointOne =
             (float4)(cx1 - params[4], cy1 - params[5], cz1 - params[6], 0);
         ...
         float cz2 = read_imagef(two5, matchPos).x;
         pointTwo =
             (float4)(cx2 - params[7], cy2 - params[8], cz2 - params[9], 0);

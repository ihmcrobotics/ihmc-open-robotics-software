...
float3 transform_point(float3 point, float3 rx, float3 ry, float3 rz,
                       float3 translation)
{
 ...
 * Center is the distance in meters from world to the center of the height map
 * region. Coordinate is the distance in meters from world to the "pixel" in the
 * height map. Therefore, we get the index of the "pixel" (x, y, bottom left
 * corner) from a world point.
 */
int coordinate_to_index(float coordinate, float center, float resolution,
                        int centerIndex)
{
...
{
   return coordinate_to_index(x, center, params[RESOLUTION],
                              params[CENTER_INDEX]);
}

int get_y_idx(float y, float center, float *params)
{
   return coordinate_to_index(y, center, params[RESOLUTION],
                              params[CENTER_INDEX]);
}
...
int get_idx_in_layer(int idx_x, int idx_y, float *params)
{
...
bool is_valid(float3 point, float3 sensor, float *params)
{
   ...
   // if (z - sz > dxy * params[RAMPED_HEIGHT_RANGE_A] +
   // params[RAMPED_HEIGHT_RANGE_C] || z - sz > params[MAX_HEIGHT_RANGE])
   //{
...
{
   return (float3)(point_in_image_frame.z, -point_in_image_frame.x,
                   -point_in_image_frame.y);
}

float3 get_point_from_image(read_only image2d_t depth_image, int x, int y,
                            global float *params)
{
   int2 key = (int2)(x, y);
   ...
   if (Z > 0.1f)
   {
      return project_from_image_frame_to_camera_frame(
          back_project_to_image_frame(key, Z, params));
   }
   else
   {
...
void kernel zeroValuesKernel(global float *params, global int *centroid_data,
...
void kernel addPointsFromImageKernel(
    read_only image2d_t depth_image, global float *localization,
    global float *params, global float *intrinsics, global int *centroid_data,
    global int *variance_data, global int *counter_data)
{
   // recall that the pixels are bottom left, and go width to height
   int image_x = get_global_id(0);
   int image_y = get_global_id(1);

   float3 point_in_camera_frame =
       get_point_from_image(depth_image, image_x, image_y, intrinsics);

   //    printf("PointInCameraFrame(%.3lf,%.3lf,%.3lf), \n",
   //    point_in_camera_frame.x, point_in_camera_frame.y,
   //    point_in_camera_frame.z);

   float3 sensor =
       (float3)(localization[tx], localization[ty], localization[tz]);
   float3 rx =
       (float3)(localization[r00], localization[r01], localization[r02]);
   float3 ry =
       (float3)(localization[r10], localization[r11], localization[r12]);
   float3 rz =
       (float3)(localization[r20], localization[r21], localization[r22]);

   float3 pointInWorld =
       transform_point(point_in_camera_frame, rx, ry, rz, sensor);
...
void kernel averageMapImagesKernel(
    global int *centroid_buffer, global int *variance_buffer,
    global int *counter_buffer, global float *params,
    write_only image2d_t centroid_x, write_only image2d_t centroid_y,
    write_only image2d_t centroid_z, write_only image2d_t variance_z,
...
void kernel computeNormalsKernel(
    read_only image2d_t centroid_x, read_only image2d_t centroid_y,
    read_only image2d_t centroid_z, read_only image2d_t counter,
    global float *params, write_only image2d_t normal_x_mat,
    write_only image2d_t normal_y_mat, write_only image2d_t normal_z_mat)
{
   int idx_x = get_global_id(0);
   int idx_y = get_global_id(1);
   ...
   if (cnt > 0)
   {
      ...
      for (int i = -params[CELLS_OVER_FOR_NORMAL];
           i <= params[CELLS_OVER_FOR_NORMAL]; i++)
      {
         if (i == 0)
            continue;
         ...
         if (mod_x > 0 && mod_x <= max_val &&
             read_imageui(counter, key_x).x > 0)
         {
         ...
         if (mod_y > 0 && mod_y <= max_val &&
             read_imageui(counter, key_y).x > 0)
         {
      ...
      // switch the x and y, because the indices are wrong for the global
      // coordinates

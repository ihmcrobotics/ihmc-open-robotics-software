                 ...
                 global float *params)
{
   ...
   if (perpDist < params[MERGE_DISTANCE_THRESHOLD] * dist * 40 &&
       sim > params[MERGE_ANGULAR_THRESHOLD])
   {
      return true;
   }
   else
...
/*
 * Replace non-reading (0s) pixels with an approximate nearby value, so they
 * don't upset the rest of the algorithm.
 */
void fillDeadPixels(read_only image2d_t inputImage, int x, int y,
                    write_only image2d_t out0, global float *params)
{
   uint Z = 0;
   int count = 0;
   ...
   for (int i = 0; i < (int)params[FILTER_KERNEL_SIZE]; i++)
   {
      for (int j = 0; j < (int)params[FILTER_KERNEL_SIZE]; j++)
      {
         ...
         /* For every unique non-zero depth value, insert x,y,z into xs,ys and
          * values, respectively. */
         if (Z != 0)
         {
            ...
            {
               /* For all other non-zero values, check against all previously
                * visited non-zero unique depth values. */
               int unique = 0;
               for (int k = 0; k < total_unique; k++)
               {
                  if ((Z - values[k]) * (Z - values[k]) >
                      params[FILTER_DISPARITY_THRESHOLD])
                  {
                     // if (x==0 && y==0)printf("Compare(%d,%d,%d)(%d,%d)\n", Z,
                     // abs(Z - values[k]), values[k], unique, total_unique);
                     unique++;
                  }
               }
               /* After checking to see if the non-zero value is truly unique
                * against stored unique values, insert.*/
   ...
   /* Fill all dead pixels in the patch with the nearest neighboring non-zero
    * unique depth value. */
   for (int i = 0; i < (int)params[FILTER_KERNEL_SIZE]; i++)
   {
      for (int j = 0; j < (int)params[FILTER_KERNEL_SIZE]; j++)
...
void smooth_non_boundary(read_only image2d_t in, int x, int y,
                         write_only image2d_t out0, global float *params)
{
   uint Z = 0;
   int m = 5;
...
{
   return (float4)(dot(r1, point) + t.x, dot(r2, point) + t.y,
                   dot(r3, point) + t.z, 0);
}
...
/*
 * Filters all pixels within a patch on depth map. Removes outliers, flying
 * points, dead pixels and measurement noise.
 */
void kernel filterKernel(read_only image2d_t inputImage,
                         write_only image2d_t filteredImage,
                         write_only image2d_t nxImage, global float *parameters)
{
   ...
   if (y >= 0 && y < filterPatchImageHeight && x >= 0 &&
       x < filterPatchImageWidth)
   {
...
/* Pack Kernel: Generates the patches by packing the centroid, surface normal
 * and metadata related to patches on a sub-sampled grid of the depth map. The
 * following intrinsic parameters are used to convert to Point Cloud. K:
 * [459.97265625, 0.0, 341.83984375, 0.0, 459.8046875, 249.173828125, 0.0,
 * 0.0, 1.0]
 * */
void kernel packKernel(
    read_only image2d_t in, write_only image2d_t out0,
    write_only image2d_t out1,
    write_only image2d_t out2, /* float3 maps for normal*/
    write_only image2d_t out3, write_only image2d_t out4,
...
)
{
   ...
   if (y >= 0 && y < (int)params[SUB_H] && x >= 0 && x < (int)params[SUB_W])
   {
      ...
      //        if(x==24 && y==50) printf("PackKernel Normal:(%.4lf, %.4lf,
      //        %.4lf)\n", normal.x, normal.y, normal.z); if(x==26 && y==7)
      //        printf("PackKernel Centroid:(%.4lf, %.4lf, %.4lf)\n",
      //        centroid.x, centroid.y, centroid.z); printf("PackKernel
...
/* Merge Kernel: Creates the graph-based structure by adding connections between
 * the neighboring patches based on similarity.
 */
void kernel mergeKernel(
    read_only image2d_t out0, read_only image2d_t out1,
    read_only image2d_t out2, /* float3 maps for normal*/
    read_only image2d_t out3, read_only image2d_t out4,
...
)
{
   ...
   if (y >= m && y < (int)params[SUB_H] - m && x >= m &&
       x < (int)params[SUB_W] - m)
   {
      ...
      //        if (n1_a + n2_a + n3_a + g1_a + g2_a + g3_a > 0 || n1_a + n2_a +
      //        n3_a + g1_a + g2_a + g3_a < 0)
      //            printf("MergeKernel: n1_a: %f, n2_a: %f, n3_a: %f, g1_a: %f,
      ...
      for (int i = -m; i < m + 1; i += m)
      {
         for (int j = -m; j < m + 1; j += m)
         {
            ...
            if (!(j == 0 && i == 0))
            {
               ...
               if (isConnected(g_a, normalize(n_a), g_b, normalize(n_b),
                               params))
               {
                  // printf("Connected: (%d,%d)\n",x+i, y+j);
                  boundaryConnectionsEncodedAsOnes =
                      (1 << count) | boundaryConnectionsEncodedAsOnes;
               }
               count++;
            }
         }
      }
      write_imageui(out6, (int2)(x, y),
                    (uint4)(boundaryConnectionsEncodedAsOnes, 0, 0, 0));
   }
}

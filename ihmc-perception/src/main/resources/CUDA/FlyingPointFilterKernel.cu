                                   ...
                                   int rows, int cols)
{
    ...
    // If the depth value deviates too much, mark as invalid (flying point removal)
    if (fabsf(depthValue - mean) > 1.5 * stdDev)
    {
        *outputPixel = 0;  // Set to zero to indicate an invalid depth point
    }
    else
    {
        *outputPixel = static_cast<unsigned short>(median);
    }
}

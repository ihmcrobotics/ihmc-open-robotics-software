...
public class PlanarRegionExtractor
{
   ...
   private void connectCellToNeighborsInRegion(PlanarRegionExtractorInputData data,
                                               int row,
                                               int column,
   ...
   public void findBoundariesAndHoles(PlanarRegionExtractorInputData data, Consumer<GPURegionRing> forDrawingDebugPanel)
   {
   ...
   private void connectAllBoundaryRingsInRegion(PlanarRegionExtractorInputData data,
   ...
   private int boundaryDepthFirstSearch(PlanarRegionExtractorInputData data,
                                        int row,
                                        int column,
                                        int planarRegionId,
   ...
   public void expandRegionBoundaries(PlanarRegionExtractorInputData data)
   {
      // this should be optional.
      Stream<GPUPlanarRegion> stream = useParallelStreams ? gpuPlanarRegions.parallelStream() : gpuPlanarRegions.stream();
      stream.forEach(planarRegion -> expandRegionBoundary(data, planarRegion));
   }

   private void expandRegionBoundary(PlanarRegionExtractorInputData data, GPUPlanarRegion planarRegion)
   {
      // this should be optional.
   ...
   public void computePlanarRegions(ReferenceFrame referenceFrameOfBackingData)
   {
      Stream<GPUPlanarRegion> stream = useParallelStreams ? gpuPlanarRegions.parallelStream() : gpuPlanarRegions.stream();
      Stream<GPUPlanarRegion> filteredStream = stream.filter(gpuPlanarRegion -> gpuPlanarRegion.getBoundaryVertices().size()
                                                                                >= polygonizerParameters.getMinNumberOfNodes());
      List<List<PlanarRegion>> listOfListsOfRegions = filteredStream.map(gpuPlanarRegion -> createPlanarRegionsFromGPUPlanarRegion(referenceFrameOfBackingData,
                                                                                                                                   gpuPlanarRegion))
   ...
   private List<PlanarRegion> createPlanarRegionsFromGPUPlanarRegion(ReferenceFrame referenceFrameOfBackingData, GPUPlanarRegion gpuPlanarRegion)
   {
      ...
      try
      {
         ...
         orientation.setIncludingFrame(referenceFrameOfBackingData, linearTransform3D.getAsQuaternion());
         orientation.changeFrame(ReferenceFrame.getWorldFrame());

         // First compute the set of concave hulls for this region
         FramePoint3D origin = new FramePoint3D(referenceFrameOfBackingData, gpuPlanarRegion.getCenter());
         ...
         List<Point2D> pointCloudInPlane = gpuPlanarRegion.getBoundaryVertices()
                                                          ...
                                                          .map(boundaryVertex ->
                                                               {
                                                                  FramePoint3D framePoint3D = new FramePoint3D(referenceFrameOfBackingData,
                                                                                                               boundaryVertex);
         ...
         for (ConcaveHull concaveHull : concaveHullCollection)
         {
            ...
            RigidBodyTransform regionTransform = new RigidBodyTransform(orientation, origin);
            PlanarRegion planarRegion = new PlanarRegion(regionTransform, concaveHull.getConcaveHullVertices(), decomposedPolygons);
   ...
   public static class PlanarRegionExtractorInputData
   {

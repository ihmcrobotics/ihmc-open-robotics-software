...
public class HeightMapUtilsTest
{
   ...
   /**
    * Test to ensure that the CUDA kernel is working when using the HeightMapUtils.cuh header file.
    * The method {@link HeightMapUtilsTest#runIndexToCoordinateOnCUDA()} returns the result from the GPU so we can compare it with the expected values.
    */
   @Test
   public void testIndexToCoordinateCUDA()
   {
      float[] result = runIndexToCoordinateOnCUDA();
   ...
   /**
    * We use the same methods in both OpenCL and in CUDA, these methods need to return the same result. This test ensures that the results that are returned
    * from the GPU are the same for each kernel.
    */
   @Test
   public void testOpenCLMatchesCUDAResult()
   {
      float[] openCLResult = runIndexToCoordinateOnOpenCL();
      float[] cudaResult = runIndexToCoordinateOnCUDA();
   ...
   private float[] runIndexToCoordinateOnCUDA()
   {

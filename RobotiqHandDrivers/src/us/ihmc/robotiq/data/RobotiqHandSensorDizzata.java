...
public class RobotiqHandSensorDizzata implements HandSensorData
{
   ...
   {
      thumbJointAngles[0] = response.getFingerAPosition().getRegisterValue() * (25.0/72) * Math.PI / 0xFF; //62.5 degrees
      thumbJointAngles[1] = response.getFingerAPosition().getRegisterValue() * (0.5) * Math.PI / 0xFF; //90 degrees
      thumbJointAngles[2] = 0.0;
      jointAngles[2] = thumbJointAngles;
      
      middleFingerJointAngles[0] = -((double)response.getScissorPosition().getRegisterValue() * (8.0/45) / 0xFF - (4.0/45)) * Math.PI; //32 degrees
      middleFingerJointAngles[1] = (double)response.getFingerBPosition().getRegisterValue() * (25.0/72) * Math.PI / 0xFF; //62.5 degrees
      middleFingerJointAngles[2] = (double)response.getFingerBPosition().getRegisterValue() * (0.5) * Math.PI / 0xFF; //90 degrees
      middleFingerJointAngles[3] = 0.0;
      jointAngles[robotSide.equals(RobotSide.LEFT) ? 1 : 0] = middleFingerJointAngles;
      
      indexFingerJointAngles[0] = ((double)response.getScissorPosition().getRegisterValue() * (8.0/45) / 0xFF - (4.0/45)) * Math.PI; //32 degrees
      indexFingerJointAngles[1] = (double)response.getFingerAPosition().getRegisterValue() * (25.0/72) * Math.PI / 0xFF; //62.5 degrees
      indexFingerJointAngles[2] = (double)response.getFingerAPosition().getRegisterValue() * (0.5) * Math.PI / 0xFF; //90 degrees

...
class TransferFunctionDiscretizerTest {
	...
	void testCausalityCheck() {
		assertThrows(IllegalArgumentException.class, ()-> {
			...
			ContinuousTransferFunction tf = new ContinuousTransferFunction("noncausal_filter", 
																			1.0,
	...
	void testCoefficientAccuracy_Order2LPButterFilter() {
		...
		// Combine input/output vectors into the same vectors for comparison.
		double[] combinedCoefficientsArray = ArrayUtils.addAll(LP_2nd_Order_Butter_Filter.getInputCoefficients().getData(), LP_2nd_Order_Butter_Filter.getOutputCoefficients().getData());
	...
	void testCoefficientAccuracy_ComplexMultiorderFilter() {
		...
		// Combine input/output vectors into the same vectors for comparison.
		double[] combinedCoefficientsArray = ArrayUtils.addAll(InvPlant_N_to_mA_Filter.getInputCoefficients().getData(), InvPlant_N_to_mA_Filter.getOutputCoefficients().getData());
	...
	void testCoefficientAccuracy_LeadLagController() {
		...
		// Combine input/output vectors into the same vectors for comparison.
		double[] combinedCoefficientsArray = ArrayUtils.addAll(Lead_Lag_Compensator_Filter.getInputCoefficients().getData(), Lead_Lag_Compensator_Filter.getOutputCoefficients().getData());

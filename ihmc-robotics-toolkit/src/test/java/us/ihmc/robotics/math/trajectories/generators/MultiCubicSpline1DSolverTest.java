   ...
   public void testGetPosition()
   {
      ...
      for (int i = 0; i < 100; i++)
      {
         ...
         for (int segmentIndex = 0; segmentIndex < numberOfWaypoints; segmentIndex++)
         {
            ...
            assertEquals(waypointPositions[firstWaypointIndex], actualPosition, 1.0e-10 * Math.max(1.0, Math.abs(expectedPosition)));
         }
      }
   }

   private static double computeExpectedPosition(double waypointTime, int waypointIndex, DMatrixRMaj solution)
   {
      DMatrixRMaj tempLine = new DMatrixRMaj(1, coefficients);
      DMatrixRMaj tempCoeffs = new DMatrixRMaj(coefficients, 1);
      MultiCubicSpline1DSolver.getPositionConstraintABlock(waypointTime, 0, 0, tempLine);

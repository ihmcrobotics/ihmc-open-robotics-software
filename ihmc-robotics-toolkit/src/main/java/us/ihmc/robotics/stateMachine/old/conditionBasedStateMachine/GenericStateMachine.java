...
/**
 * GenericStateMachine. Class for construction a finite state machine. Requires
 * an Enum, which lists all the possible state names. An YoEnum keeps track of
 * the current state and a YoDouble keeps track of the time since the state was
 * switched. A GenericStateMachine is not dynamic. New states cannot be
 * constructed on the fly. The user is given the option to set the initial state
 * in the constructor. If they do not set an initialState, then it defaults to
 * the first state in the Enum list. Setting the initialState does not call the
 * doTransitionIntoAction of that state. In order to call the
 * doTransitionIntoAction the user must call setCurrentState() instead of
 * construct with an initial state.
 *
 * @param <E>
 *            Type of Enum that lists the potential states.
 * @param <T>
 *            Type of State that is contained in the state machine.
 */
@Deprecated
public class GenericStateMachine<E extends Enum<E>, T extends State<E>>
		implements TimeInCurrentStateProvider, PreviousStateProvider<E, State<E>> {
	private static final boolean DEBUG = false;

	private final EnumMap<E, T> enumsToStates;

	private final YoEnum<E> stateYoVariable, previousStateYoVariable;
	...
	public GenericStateMachine(String name, String switchTimeName, Class<E> enumType, E initialState,
			YoDouble timeVariable, YoVariableRegistry registry) {
		this(name, switchTimeName, enumType, initialState, new YoVariableDoubleProvider(timeVariable), registry);
	}

	public GenericStateMachine(String name, String switchTimeName, Class<E> enumType, YoDouble timeVariable,
			YoVariableRegistry registry) {
		this(name, switchTimeName, enumType, null, new YoVariableDoubleProvider(timeVariable), registry);
	}

	public GenericStateMachine(String stateYoVariableName, String switchTimeName, Class<E> enumType,
			DoubleProvider timeProvider, YoVariableRegistry registry) {
		this(stateYoVariableName, switchTimeName, enumType, null, timeProvider, registry);
	}

	public GenericStateMachine(String stateYoVariableName, String switchTimeName, Class<E> enumType, E initialState,
			DoubleProvider timeProvider, YoVariableRegistry registry) {
		stateYoVariable = new YoEnum<E>(stateYoVariableName, "State machine variable to keep track of the state.",
				registry, enumType, false);
		previousStateYoVariable = new YoEnum<E>(stateYoVariableName + "PreviousState",
				"State machine variable to keep track of the previous state.", registry, enumType, true);
		...
		for (T tempState : states) {
			if (tempState.getStateEnum() == state.getStateEnum())
				throw new RuntimeException(
						"Duplicate state enums, name: " + state.getStateEnum() + ", already in use.");
		}
	...
	public void setCurrentState(E nextStateEnum) {
		if (DEBUG)
			System.out.println(getStateYoVariableName() + ": t = " + time.getValue() + ": going to state: "
					+ nextStateEnum.toString());
		...
		if (state != null) {
			...
			throw new RuntimeException("Need to add state " + nextStateEnum
					+ " to the state machine. Can't transition into the state unless it is added!");
		}
	}
	...
	public boolean checkTransitionConditions() {
		...
		// Make a Transition. First do the old states doTransitionOutOfAction, then do
		// the stateTransition do Action.
		// Then set the current state to the new state which will also do the new states
		// doTransitionIntoAction method.

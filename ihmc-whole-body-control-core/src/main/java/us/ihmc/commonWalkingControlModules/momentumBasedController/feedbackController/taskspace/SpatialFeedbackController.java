...
public class SpatialFeedbackController implements FeedbackControllerInterface
{
   ...
   protected final FeedbackControllerSettings.FilterVector3D angularVelocityErrorFilter;
   protected final FeedbackControllerSettings.FilterVector3D linearVelocityErrorFilter;
   ...
   protected final PIDSE3GainsBasics gains;
   protected final PID3DGainsBasics positionGains;
   protected final PID3DGainsBasics orientationGains;
                                    ...
                                    YoRegistry parentRegistry)
   {
      ...
      DoubleProvider maximumLinearRate = positionGains.getMaximumFeedbackRateProvider();
      DoubleProvider maximumAngularRate = orientationGains.getMaximumFeedbackRateProvider();

      controlFrame = fbToolbox.getOrCreateSpatialFeedbackControlFrame(endEffector, controllerIndex, true);

      isEnabled = new YoBoolean(FeedbackControllerToolbox.appendIndex(endEffectorName, controllerIndex) + "isSpatialFBControllerEnabled", fbToolbox.getRegistry());
      isEnabled.set(false);

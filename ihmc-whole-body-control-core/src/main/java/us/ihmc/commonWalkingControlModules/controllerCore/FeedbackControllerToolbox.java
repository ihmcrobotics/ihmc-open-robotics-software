...
public class FeedbackControllerToolbox implements FeedbackControllerDataHolderReadOnly
{
   ...
   private final Map<RigidBodyReadOnly, List<SingleFeedbackControllerDataPool>> endEffectorDataPoolMap = new HashMap<>();
    ...
    */
   public PID3DGainsBasics getOrCreateCenterOfMassGains(boolean useIntegrator, boolean isRequiredVariable)
   {
      SingleFeedbackControllerDataPool dataPool = getOrCreateCenterOfMassDataPool();
      return dataPool.getOrCreatePositionGains(useIntegrator, isRequiredVariable);
   }

   private SingleFeedbackControllerDataPool getOrCreateEndEffectorDataPool(RigidBodyReadOnly endEffector, int controllerIndex)
   {
    ...
    */
   public FBPoint3D getOrCreatePositionData(RigidBodyReadOnly endEffector, int controllerIndex, Type type, BooleanProvider activeFlag, boolean isRequiredVariable)
   {
    ...
    */
   public FBQuaternion3D getOrCreateOrientationData(RigidBodyReadOnly endEffector,
                                                    int controllerIndex,
                                                    Type type,
                                                    BooleanProvider activeFlag,
                                                    boolean isRequiredVariable)
    ...
    * @return the unique {@code FBVector3D} matching the search criteria.
    */
   public FBVector3D getOrCreateVectorData3D(RigidBodyReadOnly endEffector,
                                             int controllerIndex,
                                             Type type,
                                             SpaceData3D space,
                                             BooleanProvider activeFlag,
    ...
    * @return the unique {@code FBRateLimitedVector3D} matching the search criteria.
    */
   public FBRateLimitedVector3D getOrCreateRateLimitedVectorData3D(RigidBodyReadOnly endEffector,
                                                                   int controllerIndex,
                                                                   Type rawDataType,
                                                                   SpaceData3D space,
                                                                   double dt,
    ...
    * @return the unique {@code FBAlphaFilteredVector3D} matching the search criteria.
    */
   public FBAlphaFilteredVector3D getOrCreateAlphaFilteredVectorData3D(RigidBodyReadOnly endEffector,
                                                                       int controllerIndex,
                                                                       Type rawDataType,
                                                                       SpaceData3D space,
                                                                       double dt,
   ...
   public FilterVector3D getOrCreateAngularVelocityErrorFilter(RigidBodyReadOnly endEffector, int controllerIndex, double dt)
   {
      SingleFeedbackControllerDataPool dataPool = getOrCreateEndEffectorDataPool(endEffector, controllerIndex);
      return dataPool.getOrCreateAngularVelocityErrorFilter(dt);
   }

   public FilterVector3D getOrCreateLinearVelocityErrorFilter(RigidBodyReadOnly endEffector, int controllerIndex, double dt)
   {
      SingleFeedbackControllerDataPool dataPool = getOrCreateEndEffectorDataPool(endEffector, controllerIndex);
      return dataPool.getOrCreateLinearVelocityErrorFilter(dt);
   }
    ...
    */
   public FBPose3D getOrCreatePoseData(RigidBodyReadOnly endEffector, int controllerIndex, Type type, BooleanProvider activeFlag, boolean isRequiredVariable)
   {
    ...
    */
   public FBVector6D getOrCreateVectorData6D(RigidBodyReadOnly endEffector,
                                             int controllerIndex,
                                             Type type,
                                             SpaceData6D space,
                                             BooleanProvider activeFlag,
    ...
    */
   public FBAlphaFilteredVector6D getOrCreateAlphaFilteredVectorData6D(RigidBodyReadOnly endEffector,
                                                                       int controllerIndex,
                                                                       Type rawDataType,
                                                                       SpaceData6D space,
                                                                       double dt,
    ...
    */
   public FBRateLimitedVector6D getOrCreateRateLimitedVectorData6D(RigidBodyReadOnly endEffector,
                                                                   int controllerIndex,
                                                                   Type rawDataType,
                                                                   SpaceData6D space,
                                                                   double dt,
                                                                   DoubleProvider maximumAngularRate,
                                                                   DoubleProvider maximumLinearRate,
                                                                   BooleanProvider activeFlag,
                                                                   boolean isRequiredVariable)
   {
      SpaceData3D angularSpace = space == SpaceData6D.POSE ? SpaceData3D.ROTATION_VECTOR : space.getAngular();
    ...
    */
   public PID3DGainsBasics getOrCreateOrientationGains(RigidBodyReadOnly endEffector, int controllerIndex, boolean useIntegrator, boolean isRequiredVariable)
   {
    ...
    * @return the unique {@code YoPID3DGains} associated with the given end-effector.
    */
   public PID3DGainsBasics getOrCreatePositionGains(RigidBodyReadOnly endEffector, int controllerIndex, boolean useIntegrator, boolean isRequiredVariable)
   {
    ...
    */
   public PIDSE3GainsBasics getOrCreateSE3PIDGains(RigidBodyReadOnly endEffector, int controllerIndex, boolean useIntegrator, boolean isRequiredVariable)
   {
      PID3DGainsBasics positionGains = getOrCreatePositionGains(endEffector, controllerIndex, useIntegrator, isRequiredVariable);
      PID3DGainsBasics orientationGains = getOrCreateOrientationGains(endEffector, controllerIndex, useIntegrator, isRequiredVariable);
      return new YoPIDSE3Gains(positionGains, orientationGains);
   }
    ...
    */
   public YoSE3OffsetFrame getOrCreateSpatialFeedbackControlFrame(RigidBodyReadOnly endEffector, int controllerIndex, boolean isRequiredVariable)
   {
    ...
    */
   public YoTranslationFrame getOrCreatePointFeedbackControlFrame(RigidBodyReadOnly endEffector, int controllerIndex, boolean isRequiredVariable)
   {
    ...
    */
   public YoOrientationFrame getOrCreateOrientationFeedbackControlFrame(RigidBodyReadOnly endEffector, int controllerIndex, boolean isRequiredVariable)
   {
   ...
   @Override
   public void getPositionData(RigidBodyReadOnly endEffector, List<FBPoint3D> positionDataListToPack, Type type)
   {
      positionDataListToPack.clear();
   ...
   @Override
   public void getOrientationData(RigidBodyReadOnly endEffector, List<FBQuaternion3D> orientationDataListToPack, Type type)
   {
   ...
   @Override
   public void getVectorData(RigidBodyReadOnly endEffector, List<FBVector3D> vectorDataListToPack, Type type, SpaceData3D space)
   {
      vectorDataListToPack.clear();
      List<SingleFeedbackControllerDataPool> dataPoolList = endEffectorDataPoolMap.get(endEffector);
   ...
   private static class SingleFeedbackControllerDataPool
   {
      ...
      private PID3DGainsBasics orientationGains;
      private PID3DGainsBasics positionGains;
      ...
      public PID3DGainsBasics getOrCreateOrientationGains(boolean useIntegrator, boolean isRequiredVariable)
      {
         if (orientationGains == null)
            orientationGains = new YoPID3DGains(namePrefix + "Orientation",
                                                GainCoupling.NONE,
                                                useIntegrator,
                                                isRequiredVariable ? registry : debugRegistry);
      ...
      public PID3DGainsBasics getOrCreatePositionGains(boolean useIntegrator, boolean isRequiredVariable)
      {
         if (positionGains == null)
            positionGains = new YoPID3DGains(namePrefix + "Position", GainCoupling.NONE, useIntegrator, isRequiredVariable ? registry : debugRegistry);

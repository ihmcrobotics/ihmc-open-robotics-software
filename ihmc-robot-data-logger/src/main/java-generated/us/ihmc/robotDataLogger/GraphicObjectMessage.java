...
public class GraphicObjectMessage extends Packet<GraphicObjectMessage> implements Settable<GraphicObjectMessage>, EpsilonComparable<GraphicObjectMessage>
{
   ...
   public boolean epsilonEquals(GraphicObjectMessage other, double epsilon)
   {
      ...
      if (!us.ihmc.idl.IDLTools.epsilonEqualsIntegerSequence(this.yoVariableIndex_, other.yoVariableIndex_, epsilon))
         return false;

      if (!us.ihmc.idl.IDLTools.epsilonEqualsDoubleSequence(this.constants_, other.constants_, epsilon))
         return false;

      if (!this.appearance_.epsilonEquals(other.appearance_, epsilon))
         return false;

      if (!us.ihmc.idl.IDLTools.epsilonEqualsStringBuilder(this.listName_, other.listName_, epsilon))
         return false;
   ...
   public boolean equals(Object other)
   {
      ...
      if (other == this)
         return true;
      if (!(other instanceof GraphicObjectMessage))
         return false;
      ...
      if (!this.yoVariableIndex_.equals(otherMyClass.yoVariableIndex_))
         return false;

      if (!this.constants_.equals(otherMyClass.constants_))
         return false;

      if (!this.appearance_.equals(otherMyClass.appearance_))
         return false;

      if (!us.ihmc.idl.IDLTools.equals(this.listName_, otherMyClass.listName_))
         return false;

 ...
 * @author Sylvain
 */
public class BufferedStatusListenerManager
{
    ...
    */
   public BufferedStatusListenerManager(List<Class<? extends Command<?, ?>>> commandsToRegister)
   {
    ...
    * @param commandsToRegister list of the commands that this API should support.
    */
   public BufferedStatusListenerManager(String name, List<Class<? extends Command<?, ?>>> commandsToRegister)
   {
    ...
    */
   public BufferedStatusListenerManager(String name, List<Class<? extends Command<?, ?>>> commandsToRegister, int buffersCapacity)
   {
      this.printStatementPrefix = name == null ? "" : name + ": ";
      this.buffersCapacity = buffersCapacity;
      registerNewMessages(commandsToRegister);
   }
   ...
   private <C extends Settable<C>, M extends Settable<M>> void registerNewMessages(List<Class<? extends Settable<?>>> messageClasses)
   {
      for (int i = 0; i < messageClasses.size(); i++)
         registerNewMessage((Class<C>) messageClasses.get(i));
   }
   ...
   private <M extends Settable<M>> void registerNewMessage(Class<M> messageClass)
   {
      ...
      Class<M> messageClass = builer.newInstance().getClass();
      messageClassToBufferMap.put(messageClass, newBuffer);
      commandsMap.put(messageClass, new RecyclingArrayList<>(buffersCapacity, messageClass));
   ...
   private void submitMessageInternal(Settable message)
   {
      ...
      Settable nextMessage = (Settable) buffer.next();
      if (nextMessage == null)
      {
      ...
      Class<?> messageClass = nextMessage.getClass();

      try
      {
         if (!performCustomConversion(message, messageClass))
         {
      ...
      for (int i = 0; i < hasReceivedInputListeners.size(); i++)
         hasReceivedInputListeners.get(i).hasReceivedInput((Class<? extends Settable<?>>) messageClass);
   }
    ...
    */
   public <C extends Command<C, ?>> void clearMessages(Class<C> messageClassToCLear)
   {
    ...
    */
   public <C extends Command<C, ?>> C pollNewestMessage(Class<C> commandClassToPoll)
   {
      return ((RecyclingArrayList<C>) pollNewMessages(commandClassToPoll)).getLast();
   }
    ...
    */
   @SuppressWarnings("unchecked")
   public <C extends Command<C, ?>> List<C> pollNewMessages(Class<C> commandClassToPoll)
   {
      RecyclingArrayList<C> messages = (RecyclingArrayList<C>) commandsMap.get(commandClassToPoll);
      ConcurrentRingBuffer<C> buffer = (ConcurrentRingBuffer<C>) messageClassToBufferMap.get(commandClassToPoll);

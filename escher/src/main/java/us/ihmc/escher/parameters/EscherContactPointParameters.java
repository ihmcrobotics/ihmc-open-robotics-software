...
public class EscherContactPointParameters extends RobotContactPointParameters
{
   private final SideDependentList<ArrayList<Point2D>> footGroundContactPoints = new SideDependentList<>();

   private final DRCRobotJointMap jointMap;

   public EscherContactPointParameters(DRCRobotJointMap jointMap)
   {
      super(jointMap, EscherPhysicalProperties.footWidthForControl, EscherPhysicalProperties.footLengthForControl, EscherPhysicalProperties.soleToAnkleFrameTransforms);
      ...
      for (Collision collision : link.getCollisions())
      {
         ...
         if (name.contains("ctrl_contact"))
         {
            ...
            for (RobotSide robotSide : RobotSide.values)
            {
               ...
               {
                  footGroundContactPoints.get(robotSide).add(projectOnPlane(EscherPhysicalProperties.getSoleToAnkleFrameTransform(robotSide), gcOffset));
                  assigned = true;
                  break;
               }

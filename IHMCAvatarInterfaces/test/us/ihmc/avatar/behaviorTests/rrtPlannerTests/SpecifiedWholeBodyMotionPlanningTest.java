...
public abstract class SpecifiedWholeBodyMotionPlanningTest implements MultiRobotTestInterface
{
   ...
   public void testForposeForNode() throws SimulationExceededMaximumTimeException, IOException
   {
      ...
      HandTrajectoryMessage handTrajectoryMessage = new HandTrajectoryMessage(RobotSide.RIGHT, motionTime, point3D, quaternion,
   ...
   public void testForPushDoorKinematics() throws SimulationExceededMaximumTimeException, IOException
   {
      ...
      FramePoint3D desiredPointToWorld = new FramePoint3D(referenceFrames.getWorldFrame(), desiredPose.getPosition());
      ...
      desiredPointToWorld = new FramePoint3D(referenceFrames.getWorldFrame(), desiredPose.getPosition());
   ...
   public void testForPlannerPushDoorOpeningTask() throws SimulationExceededMaximumTimeException, IOException
   {
      ...
      SimulationConstructionSet scs = drcBehaviorTestHelper.getSimulationConstructionSet();
      /*
       * Motion planning node. Input @param. solar panel position and
       * orientation. cleaning path. - robot side, via points, type(linear,
       * circular_not yet). trajectory time for initial position to root node
       * pose. finding initial node. Output @param. wholebody trajectory
       * message. -> hand trajectory message(end effector) is obtained from end
       * effector path. -> chest and pelvis trajectory message is obtained from
       * planner.
       */

...
public abstract class RRTNode implements RRTInterface 
{
   ...
   protected final int getDimensionOfNodeData()
   {
      return nodeData.getDimension();
   }

   protected final double getNodeData(int index)
   {
      return nodeData.getQ(index);
   }

   protected final double getDistance(RRTNode targetNode)
   {
      return nodeData.distance(targetNode.nodeData);
   }

   protected final void setNodeData(int index, double data)
   {
      nodeData.setQ(index, data);
   }

   protected final void addChildNode(RRTNode childNode)
   {
      childRRTNode.add(childNode);
      childNode.setParentNode(this);
   }

   protected final void setChildNode(int indexOfChild, RRTNode childNode)
   {
      childRRTNode.set(indexOfChild, childNode);
   }

   protected final RRTNode getChildNode(int indexOfChild)
   {
      return childRRTNode.get(indexOfChild);
   }

   protected final int getNumberOfChild()
   {
      return childRRTNode.size();
   }

   protected final void setParentNode(RRTNode parentNode)
   {
      parentRRTNode = parentNode;
   }

   protected final RRTNode getParentNode()
   {

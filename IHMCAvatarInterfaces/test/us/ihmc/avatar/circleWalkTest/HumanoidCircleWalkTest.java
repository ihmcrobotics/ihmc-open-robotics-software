...
import us.ihmc.simulationConstructionSetTools.util.globalParameters.SystemOutGlobalParameterChangedListener;
...
public abstract class HumanoidCircleWalkTest implements MultiRobotTestInterface{
	...
	public void testCircleWalk() throws SimulationExceededMaximumTimeException
	{
		...
		for(int armJointIndex=0; armJointIndex < getArmDoF(); armJointIndex++)
		{
			...
			for(int trajectoryPointIndex =0; trajectoryPointIndex < getArmTrajectoryPoints(); trajectoryPointIndex++)
			{
				...
				leftJointTrajectory.setTrajectoryPoint(trajectoryPointIndex, 2*trajectoryPointIndex +1, getRandomValidJointAngle(RobotSide.LEFT, armJoint[armJointIndex]), 0);
				rightJointTrajectory.setTrajectoryPoint(trajectoryPointIndex, 2*trajectoryPointIndex +1, getRandomValidJointAngle(RobotSide.RIGHT, armJoint[armJointIndex]), 0);
			}
		...
		while(theta < 2*Math.PI)
		{
			...
			{
				Point3D footLocation = new Point3D((radius + side.negateIfLeftSide(stepWidth/2))*Math.cos(theta), (radius + side.negateIfLeftSide(stepWidth/2))*Math.sin(theta), 0.0);
	...
	private class ControllerSpy extends SimpleRobotController
	{
		...
		/*private BufferedWriter fileDump;
		...
		public ControllerSpy(DRCSimulationTestHelper drcSimulationTestHelper)
		{
			...
			/*try{
			fileDump = new BufferedWriter(new FileWriter(new File("nameDump")));}
			catch (Exception e) {
		...
		public void doControl()
		{
			...
			// Taking a dump of the YoVariable Names
			/*if(runOnce)
			{
				ArrayList<YoVariable<?>> varList = humanoidRobotModel.getAllVariables();
				for (int i = 0; i < varList.size(); i++) {
					try{
					fileDump.write(varList.get(i).getName() + "\n");}
					catch(Exception e){

...
public class LopingSpeedController implements RobotController
{
	private final YoVariableRegistry registry = new YoVariableRegistry("controller");

	private enum States
	{
		SUPPORT, PUSH_OFF, SWING, STOP
	};
	...
	private final YoVariable speed = new YoVariable("speed", registry);
	private YoVariable t, qd_x;
	...
	LopingSpeedController(LopingHopperRobot robot, double gravity)
	{
		this.robot = robot;
		this.gravity = gravity;
		t = robot.getVariable("t");
		qd_x = robot.getVariable("qd_x");
		speed.val = 1;
		...
		if(moon)
		{
			...
			frontHipForword.val = -0.4;
			frontHipBackword.val = -0.3;
			...
			backLegLengthExtend.val = -0.0;
			backLegLengthRetract.val = 0.2;
			min_support_time.val = 0.0;
			qd_x.val = 0.3;
		...
		else
		{
			...
			leg_gain.val = 10000.0;
			leg_damp.val = 300.0;

			hip_gain.val = 10000.0;
			hip_damp.val = 100.0;
	...
	private void setupStateMachines()
	{
		...
		State frontStopState = new State(States.STOP)
		{
			public void doAction()
			{
				q_d_frontLegJoint.val = frontLegLengthRetract.val;
				...
				q_d_frontHipJoint.val = frontHipForword.val;
				qd_d_frontHipJoint.val = 0.0;
			}
		...
		State backStopState = new State(States.STOP)
		{
			public void doAction()
			{
				q_d_backLegJoint.val = backLegLengthRetract.val;
				...
				q_d_backHipJoint.val = backHipForword.val;;
				qd_d_backHipJoint.val = 0.0;
			}
		...
		StateTransitionCondition stop = new StopCondition();
		StateTransitionCondition go = new GoCondition();
		...
		StateTransition backSupportToStop = new StateTransition(States.STOP, stop);
		backSupportState.addStateTransition(backSupportToStop);
		...
		StateTransition frontSupportToStop = new StateTransition(States.STOP, stop);
		frontSupportState.addStateTransition(frontSupportToStop);
		...
		StateTransition stopToGo = new StateTransition(States.SUPPORT, go);
		frontStopState.addStateTransition(stopToGo);
		backStopState.addStateTransition(stopToGo);
		...
		backStateMachine.addState(backStopState);
		...
		frontStateMachine.addState(frontStopState);
	...
	public class StopCondition implements StateTransitionCondition
	{
	...
	public class GoCondition implements StateTransitionCondition
	{
		public boolean checkCondition()
		{
			return (speed.val!=0);

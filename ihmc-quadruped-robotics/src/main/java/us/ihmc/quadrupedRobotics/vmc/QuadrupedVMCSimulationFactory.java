...
import us.ihmc.quadrupedRobotics.inverseKinematics.QuadrupedInverseKinematicsController;
...
public class QuadrupedVMCSimulationFactory
{
   ...
   private QuadrupedVMCController vmcController;
   ...
   {
      vmcController = new QuadrupedVMCController(fullRobotModel.get(), quadrants.get(), jointDesiredOutputList.get(), optimizationSettings.get(),
                                                 controlDT.get(), gravity.get(), yoGraphicsListRegistry);
   }

   private void createSimulationController()
   {
      simulationController = new QuadrupedSimulationController(sdfRobot.get(), sensorReader, outputWriter.get(), vmcController, null, null, null);
   }

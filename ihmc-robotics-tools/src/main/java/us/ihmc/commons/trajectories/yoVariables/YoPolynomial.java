...
public class YoPolynomial implements PolynomialVariableHolder, PolynomialBasics
{
   ...
   public YoPolynomial(String name, double[] values, YoRegistry registry)
   {
      this(createCoefficientsArray(name, values, registry),
           new YoInteger(name + "_nCoeffs", registry),
           new YoDouble(name + "_tCurrent", registry));
   }
                       ...
                       YoDouble currentTime)
   {
      ...
      derivativeCoefficientVector = new DMatrixRMaj(maximumNumberOfCoefficients, 1);
      doubleDerivativeCoefficientVector = new DMatrixRMaj(maximumNumberOfCoefficients, 1);
   ...
   private static YoDouble[] createCoefficientsArray(String name, double[] values, YoRegistry registry)
   {
      if (values.length < 1)
         throw new IllegalArgumentException("You have to make the polynomial to have at least 1 coefficient!");

      YoDouble[] a = new YoDouble[values.length];
   ...
   {
      return derivativeCoefficientVector.get(i);
   }

   @Override
   public double getDoubleDerivativeCoefficient(int i)
   {
      return doubleDerivativeCoefficientVector.get(i);
   }
   ...
   public void compute(double x)
   {
      ...
      for (int i = 0; i < numberOfCoefficients.getIntegerValue() - 1; i++)
         vel += derivativeCoefficientVector.get(i) * xPowers[i];
      for (int i = 0; i < numberOfCoefficients.getIntegerValue() - 2; i++)
         acc += doubleDerivativeCoefficientVector.get(i) * xPowers[i];
   ...
   public void reshape(int numberOfCoefficientsRequired)
   {
      ...
      derivativeCoefficientVector.reshape(numberOfCoefficientsRequired, 1);
   ...
   public void setCoefficientUnsafe(int idx, double value)
   {
      a[idx].set(value);
      if (idx > 0)
         derivativeCoefficientVector.set(idx - 1, value * idx);
      if (idx > 1)
         doubleDerivativeCoefficientVector.set(idx - 2, value * (idx - 1) * idx);
   }

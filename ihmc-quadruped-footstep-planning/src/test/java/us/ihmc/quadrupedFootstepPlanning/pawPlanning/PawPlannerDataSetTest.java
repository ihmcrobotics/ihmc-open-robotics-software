...
public abstract class PawPlannerDataSetTest
{
   ...
   public void testSimpleForwardPoint()
   {
      ...
      messager.submitMessage(PawPlannerMessagerAPI.XGaitSettingsTopic, xGaitSettings);
      messager.submitMessage(PawPlannerMessagerAPI.PlanarRegionDataTopic, null);
      messager.submitMessage(PawPlannerMessagerAPI.StartPositionTopic, new Point3D(startPose.getPosition()));
      messager.submitMessage(PawPlannerMessagerAPI.GoalPositionTopic, new Point3D(goalPose.getPosition()));
      messager.submitMessage(PawPlannerMessagerAPI.StartOrientationTopic, new Quaternion(startPose.getOrientation()));
      messager.submitMessage(PawPlannerMessagerAPI.GoalOrientationTopic, new Quaternion(goalPose.getOrientation()));
      //      planner.setHorizonLengthTopic(Double.MAX_VALUE);

      PawPlanningResult pathResult = planner.planPath();
      assertTrue("Path plan is invalid. Got path result " + pathResult, pathResult.validForExecution());

      PawPlanningResult planResult = planner.plan();
      assertTrue("Footstep plan is invalid. Got path result " + planResult, planResult.validForExecution());

      messager.submitMessage(PawPlannerMessagerAPI.FootstepPlanTopic, planner.getPlan());
...
//
   ...
   private static String assertPlanIsValid(String datasetName, PawPlan plannedSteps, Point3DReadOnly goalPosition, double goalYaw)
   {
   ...
   private static String checkStepOrder(String dataseName, PawPlan plannedSteps)
   {
      String errorMessage = "";
   ...
   private void visualizePlan(PawPlan plan, PlanarRegionsList planarRegionsList, Point3DReadOnly start, Point3DReadOnly goal)
   {

...
//      mpc.compute(0.0);
      ...
      List<OrientationTrajectoryCommand> orientationTrackingCommands = new ArrayList<>();
      List<ForceTrackingCommand> forceTrackingCommands = new ArrayList<>();
      List<RhoBoundCommand> rhoBoundCommands = new ArrayList<>();
      List<RhoTrackingCommand> rhoTrackingCommands = new ArrayList<>();
      List<RhoRateTrackingCommand> rhoRateTrackingCommands = new ArrayList<>();
      for (int i = 0; i < mpc.mpcCommands.getNumberOfCommands(); i++)
      {
         ...
         else if (command instanceof VRPTrackingCommand)
            vrpTrackingCommands.add((VRPTrackingCommand) command);
         else if (command instanceof  OrientationTrajectoryCommand)
            orientationTrackingCommands.add((OrientationTrajectoryCommand) command);
         else if (command instanceof  ForceTrackingCommand)
            forceTrackingCommands.add((ForceTrackingCommand) command);
         else if (command instanceof  RhoBoundCommand)
            rhoBoundCommands.add((RhoBoundCommand) command);
         else if (command instanceof  RhoTrackingCommand)
            rhoTrackingCommands.add((RhoTrackingCommand) command);
         else if (command instanceof  RhoRateTrackingCommand)
            rhoRateTrackingCommands.add((RhoRateTrackingCommand) command);
      }
      ...
      {
         LogTools.info("Adding final COM");
      ...
      assertEquals(mpc.previewWindowCalculator.getPlanningWindow().size(), orientationTrackingCommands.size());
      if (MPCParameters.includeForceMinimization)
         assertEquals(2 * (mpc.previewWindowCalculator.getPlanningWindow().size() - 1), forceTrackingCommands.size());
      assertEquals(mpc.previewWindowCalculator.getPlanningWindow().size() - 1, rhoTrackingCommands.size());
      assertEquals(mpc.previewWindowCalculator.getPlanningWindow().size() - 1, rhoRateTrackingCommands.size());
      assertEquals(mpc.previewWindowCalculator.getPlanningWindow().size() - 1, rhoBoundCommands.size());
      ...
      for (int i = 0; i < mpc.previewWindowCalculator.getPlanningWindow().size(); i++)
      {
         int segmentToMatch = i;

         if (mpc.previewWindowCalculator.getPlanningWindow().get(i).getContactState().isLoadBearing())
         {
            assertTrue(rhoBoundCommands.stream().anyMatch(command -> command.getSegmentNumber() == segmentToMatch));
            assertTrue(rhoTrackingCommands.stream().anyMatch(command -> command.getSegmentNumber() == segmentToMatch));
            assertTrue(rhoRateTrackingCommands.stream().anyMatch(command -> command.getSegmentNumber() == segmentToMatch));

            if (MPCParameters.includeForceMinimization)
            {
               int numberShouldMatch = mpc.previewWindowCalculator.getPlanningWindow().get(i).getNumberOfContactPlanes();
               assertEquals(numberShouldMatch, forceTrackingCommands.stream().filter(command -> command.getSegmentNumber() == segmentToMatch).count());
            }
         }
         assertTrue(orientationTrackingCommands.stream().anyMatch(command -> command.getSegmentNumber() == segmentToMatch));

         if (i < mpc.previewWindowCalculator.getPlanningWindow().size() - 1)
         {
            assertTrue(comPositionContinuityCommands.stream().anyMatch(command -> command.getFirstSegmentNumber() == segmentToMatch));
            assertTrue(comVelocityContinuityCommands.stream().anyMatch(command -> command.getFirstSegmentNumber() == segmentToMatch));
            assertTrue(orientationContinuityCommands.stream().anyMatch(command -> command.getSegmentNumber() == segmentToMatch));
         }
      }

      if (MPCParameters.includeFinalCoMPositionObjective)
      {
         CoMPositionCommand finalCoMPositionCommand = comPositionCommands.stream()
                                                                         .filter(command -> command.getSegmentNumber() == mpc.previewWindowCalculator.getPlanningWindow().size() - 1)
                                                                         .findFirst()
                                                                         .get();
         assertEquals(MPCParameters.finalCoMPositionConstraintType, finalCoMPositionCommand.getConstraintType());
      }
      if (MPCParameters.includeFinalDCMPositionObjective)
      {
         DCMPositionCommand finalCoMPositionCommand = dcmPositionCommands.stream()
                                                                         .filter(command -> command.getSegmentNumber() == mpc.previewWindowCalculator.getPlanningWindow().size() - 1)
                                                                         .findFirst()
                                                                         .get();
         assertEquals(MPCParameters.finalDCMPositionConstraintType, finalCoMPositionCommand.getConstraintType());
      }
...
//      inputCalculator.calculateCoMContinuityObjectiveInternal()
      ...
      if (MPCParameters.includeFinalCoMPositionObjective && MPCParameters.finalCoMPositionConstraintType == ConstraintType.EQUALITY)
      {
         ...
         int rhoStart = indexHandler.getRhoCoefficientStartIndex(segmentNumber);
         for (int i = 0; i < contactPlanes.size(); i++)
         {
            ContactPlaneJacobianCalculator.computeLinearJacobian(0, segmentDuration, mpc.omega.getValue(), rhoStart, contactPlanes.get(i), tempJacobian);
            rhoStart += contactPlanes.get(i).getCoefficientSize();
         }

         MatrixTools.setMatrixBlock(linearConstraintMatrix, row, 0, tempJacobian, 0, 0, 3, indexHandler.getTotalProblemSize(), 1.0);

         mpc.comPositionAtEndOfWindow.get(row, linearConstraintVector);
         linearConstraintVector.add(row + 2, 0, 0.5 * 9.81 * segmentDuration * segmentDuration);
      ...
      if (MPCParameters.includeFinalCoMVelocityObjective && MPCParameters.finalCoMVelocityConstraintType == ConstraintType.EQUALITY)
      {
         ...
         List<MPCContactPlane> contactPlanes = mpc.contactHandler.getContactPlanesForSegment(segmentNumber);
         CoMCoefficientJacobianCalculator.calculateCoMJacobian(indexHandler.getComCoefficientStartIndex(segmentNumber), segmentDuration, tempJacobian, 1, 1.0);
         int rhoStart = indexHandler.getRhoCoefficientStartIndex(segmentNumber);
         for (int i = 0; i < contactPlanes.size(); i++)
         {
            ContactPlaneJacobianCalculator.computeLinearJacobian(1, segmentDuration, mpc.omega.getValue(), rhoStart, contactPlanes.get(i), tempJacobian);
            rhoStart += contactPlanes.get(i).getCoefficientSize();
         }

         MatrixTools.setMatrixBlock(linearConstraintMatrix, row, 0, tempJacobian, 0, 0, 3, indexHandler.getTotalProblemSize(), 1.0);

         mpc.comVelocityAtEndOfWindow.get(row, linearConstraintVector);
         linearConstraintVector.add(row + 2, 0, 9.81 * segmentDuration);
      ...
      if (MPCParameters.includeFinalDCMPositionObjective && MPCParameters.finalDCMPositionConstraintType == ConstraintType.EQUALITY)
      {
         ...
         List<MPCContactPlane> contactPlanes = mpc.contactHandler.getContactPlanesForSegment(segmentNumber);
         CoMCoefficientJacobianCalculator.calculateCoMJacobian(indexHandler.getComCoefficientStartIndex(segmentNumber), segmentDuration, tempJacobian, 0, 1.0);
         CoMCoefficientJacobianCalculator.calculateCoMJacobian(indexHandler.getComCoefficientStartIndex(segmentNumber), segmentDuration, tempJacobian, 1, 1.0 / mpc.omega.getValue());
         int rhoStart = indexHandler.getRhoCoefficientStartIndex(segmentNumber);
         for (int i = 0; i < contactPlanes.size(); i++)
         {
            ContactPlaneJacobianCalculator.computeLinearJacobian(0, segmentDuration, mpc.omega.getValue(), rhoStart, contactPlanes.get(i), tempJacobian);
            ContactPlaneJacobianCalculator.computeLinearJacobian(1.0 / mpc.omega.getValue(), 1, segmentDuration, mpc.omega.getValue(), rhoStart, contactPlanes.get(i), tempJacobian);
            rhoStart += contactPlanes.get(i).getCoefficientSize();
         }

         MatrixTools.setMatrixBlock(linearConstraintMatrix, row, 0, tempJacobian, 0, 0, 3, indexHandler.getTotalProblemSize(), 1.0);

         FramePoint3D dcmAtEndOfState = new FramePoint3D();
         dcmAtEndOfState.scaleAdd(1.0 / mpc.omega.getValue(), mpc.comVelocityAtEndOfWindow, mpc.comPositionAtEndOfWindow);
         EuclidFrameTestTools.assertFramePoint3DGeometricallyEquals(dcmAtEndOfState, mpc.dcmAtEndOfWindow, 1e-5);

         dcmAtEndOfState.get(row, linearConstraintVector);
         linearConstraintVector.add(row + 2, 0, 0.5 * 9.81 * segmentDuration * segmentDuration + 9.81 / mpc.omega.getValue() * segmentDuration);
      ...
      if (MPCParameters.finalVRPPositionConstraintType == ConstraintType.EQUALITY)
      {
         ...
         List<MPCContactPlane> contactPlanes = mpc.contactHandler.getContactPlanesForSegment(segmentNumber);
         CoMCoefficientJacobianCalculator.calculateCoMJacobian(indexHandler.getComCoefficientStartIndex(segmentNumber), segmentDuration, tempJacobian, 0, 1.0);
         CoMCoefficientJacobianCalculator.calculateCoMJacobian(indexHandler.getComCoefficientStartIndex(segmentNumber), segmentDuration, tempJacobian, 2, -1.0 / MathTools.square(mpc.omega.getValue()));
         int rhoStart = indexHandler.getRhoCoefficientStartIndex(segmentNumber);
         for (int i = 0; i < contactPlanes.size(); i++)
         {
            ContactPlaneJacobianCalculator.computeLinearJacobian(0, segmentDuration, mpc.omega.getValue(), rhoStart, contactPlanes.get(i), tempJacobian);
            ContactPlaneJacobianCalculator.computeLinearJacobian(-1.0 / MathTools.square(mpc.omega.getValue()),
                                                                 2,
                                                                 segmentDuration,
                                                                 mpc.omega.getValue(),
                                                                 rhoStart,

...
//         yoGraphicsListRegistry.registerYoGraphicsList(graphicsList);
   ...
   private void setDCMTerminalConstraint(int sequenceId, FramePoint3DReadOnly terminalDCMPosition)
   {
      ...
      if (contactMotion.getNumberOfCoefficients() > 3)
      {
         c2 = Math.min(CoMTrajectoryPlannerTools.sufficientlyLarge, duration);
         c3 = 1;
      }
      else
      {
   ...
   private void setVRPStartPosition(int sequenceId)
   {
      ...
      if (contactState.isLoadBearing())
      {
         ...
         FramePoint3DReadOnly vrpPosition = startVRPPositions.get(sequenceId);
         vrpPosition.checkReferenceFrameMatch(worldFrame);
         ...
         xCoefficientConstants.set(numberOfConstraints, 0, vrpPosition.getX());
         yCoefficientConstants.set(numberOfConstraints, 0, vrpPosition.getY());
         zCoefficientConstants.set(numberOfConstraints, 0, vrpPosition.getZ());
   ...
   private void setVRPEndPosition(int sequenceId)
   {
      ...
      if (contactState.isLoadBearing() && contactStateProvider.getContactMotion().getNumberOfCoefficients() > 3)
      {
         ...
         FramePoint3DReadOnly vrpPosition = endVRPPositions.get(sequenceId);
         vrpPosition.checkReferenceFrameMatch(worldFrame);

...
public class ModifiedOptimizedCoMTrajectoryPlanner implements CoMTrajectoryProvider
{
   ...
   private void solveForCoefficientConstraintMatrix(List<? extends ContactStateProvider> contactSequence)
   {
      ...
      addCoMPositionObjective(HIGH_WEIGHT, currentCoMPosition);
      if (includeVelocityObjective && maintainInitialCoMVelocityContinuity.getBooleanValue())
         addCoMVelocityObjective(MEDIUM_WEIGHT, currentCoMVelocity);
      addDynamicsInitialObjective(HIGH_WEIGHT, MEDIUM_WEIGHT, contactSequence, 0);

      // add transition continuity constraints
      int transition = 0;
      {
         addCoMPositionContinuityObjective(CONSTRAINT_WEIGHT, contactSequence, 0, 1);
         addCoMVelocityContinuityObjective(CONSTRAINT_WEIGHT, contactSequence, 0, 1);
         addDynamicsFinalObjective(LOW_WEIGHT, VERY_LOW_WEIGHT, contactSequence, 0);
         addDynamicsInitialObjective(LOW_WEIGHT, VERY_LOW_WEIGHT, contactSequence, 1);
      }
      transition++;
      for (; transition < numberOfTransitions; transition++)
      {
         ...
         addCoMPositionContinuityObjective(CONSTRAINT_WEIGHT, contactSequence, previousSequence, nextSequence);
         addCoMVelocityContinuityObjective(CONSTRAINT_WEIGHT, contactSequence, previousSequence, nextSequence);
         addDynamicsFinalObjective(MEDIUM_WEIGHT, LOW_WEIGHT, contactSequence, previousSequence);
         addDynamicsInitialObjective(MEDIUM_WEIGHT, LOW_WEIGHT, contactSequence, nextSequence);
      }
      ...
      addDCMPositionObjective(HIGH_WEIGHT, numberOfPhases - 1, finalDuration, finalDCMPosition);
      addDynamicsFinalObjective(HIGH_WEIGHT, MEDIUM_WEIGHT, contactSequence, numberOfPhases - 1);
...
//         trajectory3D.setLinear(0.0, duration, vrpStartPosition, vrpEndPosition);
   ...
   {
      CoMTrajectoryPlannerTools.addCoMPositionObjective(weight, centerOfMassLocationForConstraint, omega.getValue(), 0.0, 0,
   ...
   {
      CoMTrajectoryPlannerTools.addCoMVelocityObjective(weight, desiredCoMVelocity, omega.getValue(), 0.0, 0,
                                                        hessian, xGradient, yGradient, zGradient);
   }
   ...
   {
      CoMTrajectoryPlannerTools.addDCMPositionObjective(weight, desiredDCMPosition, omega.getValue(), time, sequenceId, hessian, xGradient, yGradient, zGradient);
   }
                                                  ...
                                                  int nextSequence)
   {
      ...
      CoMTrajectoryPlannerTools.addCoMPositionContinuityObjective(weight, previousSequence, nextSequence, omega.getValue(), previousDuration, hessian);
   }
   ...
   {
      double previousDuration = contactSequence.get(previousSequence).getTimeInterval().getDuration();
      CoMTrajectoryPlannerTools.addCoMVelocityContinuityObjective(weight, previousSequence, nextSequence, omega.getValue(), previousDuration, hessian);
   }
   ...
   private void addDynamicsInitialObjective(double positionWeight, double velocityWeight, List<? extends ContactStateProvider> contactSequence, int sequenceId)
   {
      ...
      if (contactState.isLoadBearing())
      {
         ...
         addVRPPositionObjective(positionWeight, sequenceId, 0.0, startVRPPositions.get(sequenceId));
         addVRPVelocityObjective(velocityWeight, sequenceId, 0.0, desiredVelocity);
      }
      else
      {
         addCoMAccelerationIsGravityObjective(CONSTRAINT_WEIGHT, sequenceId, 0.0);
         addZeroCoMJerkObjective(CONSTRAINT_WEIGHT, sequenceId, 0.0);
      }
   }
   ...
   private void addDynamicsFinalObjective(double positionWeight, double velocityWeight, List<? extends ContactStateProvider> contactSequence, int sequenceId)
   {
      ...
      if (contactState.isLoadBearing())
      {
         desiredVelocity.sub(endVRPPositions.get(sequenceId), startVRPPositions.get(sequenceId));
         desiredVelocity.scale(1.0 / duration);
         addVRPPositionObjective(positionWeight, sequenceId, duration, endVRPPositions.get(sequenceId));
         addVRPVelocityObjective(velocityWeight, sequenceId, duration, desiredVelocity);
      }
      else
      {
         addCoMAccelerationIsGravityObjective(CONSTRAINT_WEIGHT, sequenceId, duration);
         addZeroCoMJerkObjective(CONSTRAINT_WEIGHT, sequenceId, duration);
      }
   }
   ...
   {
      CoMTrajectoryPlannerTools.addVRPPositionObjective(weight, desiredVRPPosition, omega.getValue(), time, sequenceId,
                                                        hessian, xGradient, yGradient, zGradient);
   }
   ...
   {
      CoMTrajectoryPlannerTools.addVRPVelocityObjective(weight, desiredVRPVelocity, omega.getValue(), time,  sequenceId, hessian, xGradient, yGradient, zGradient);
   }
   ...
   {
      CoMTrajectoryPlannerTools.addCoMAccelerationIsGravityObjective(weight, sequenceId, omega.getValue(), time, gravityZ, hessian, xGradient);
   }
   ...
   {
      CoMTrajectoryPlannerTools.addCoMJerkObjective(weight, null, omega.getValue(),  time, sequenceId, hessian, null, null, null);
   }

...
public class WholeBodyControllerBoundCalculator {
	...
	private final YoBoolean areJointVelocityLimitsConsidered = new YoBoolean("areJointVelocityLimitsConsidered",
			registry);
	...
	public WholeBodyControllerBoundCalculator(JointIndexHandler jointIndexHandler, double controlDT,
			boolean considerJointVelocityLimits, YoRegistry parentRegistry) {
		...
		for (OneDoFJointBasics joint : jointIndexHandler.getIndexedOneDoFJoints()) {
			...
			filterAlpha.set(AlphaFilteredYoVariable.computeAlphaGivenBreakFrequencyProperly(Double.POSITIVE_INFINITY,
					controlDT));
			AlphaFilteredYoVariable filteredLowerLimit = new AlphaFilteredYoVariable(
					"qdd_min_filter_" + joint.getName(), registry, filterAlpha);
			AlphaFilteredYoVariable filteredUpperLimit = new AlphaFilteredYoVariable(
					"qdd_max_filter_" + joint.getName(), registry, filterAlpha);
	...
	/**
	 * Sets whether the joint velocity limits as defined by
	 * {@link OneDoFJoint#getVelocityLimitLower()} and
	 * {@link OneDoFJoint#getVelocityLimitUpper()} should be considered.
	 * 
	 * @param value if {@code true} the joint velocity limits will be considered, if
	 *              {@code false} they will be ignored.
	 */
	...
	public void submitJointLimitEnforcementMethodCommand(JointLimitEnforcementMethodCommand command) {
		for (int idx = 0; idx < command.getNumberOfJoints(); idx++) {
			...
			if (params != null) {
				...
				filterAlphas[jointIndex].set(AlphaFilteredYoVariable.computeAlphaGivenBreakFrequencyProperly(
						params.getJointLimitFilterBreakFrequency(), controlDT));
	...
	private void computeVelocityLimitDefault(OneDoFJointBasics joint, DMatrixRMaj qDotMinToPack,
			DMatrixRMaj qDotMaxToPack) {
	...
	public void computeJointAccelerationLimits(double absoluteMaximumJointAcceleration, DMatrixRMaj qDDotMinToPack,
		...
		for (OneDoFJointBasics joint : oneDoFJoints) {
			...
			switch (jointLimitTypes[jointIndex]) {
			case DEFAULT:
				computeAccelerationLimitDefault(joint, absoluteMaximumJointAcceleration, qDDotMinToPack,
						qDDotMaxToPack);
				break;
			case RESTRICTIVE:
				computeAccelerationLimitRestrictive(joint, absoluteMaximumJointAcceleration, qDDotMinToPack,
						qDDotMaxToPack);
				break;
			case NONE:
				break;
	...
	private void computeAccelerationLimitDefault(OneDoFJointBasics joint, double absoluteMaximumJointAcceleration,
			DMatrixRMaj qDDotMinToPack, DMatrixRMaj qDDotMaxToPack) {
		int index = jointIndexHandler.getOneDoFJointIndex(joint);
		double jointLimitLower = jointLowerLimits.get(index, 0);
		double jointLimitUpper = jointUpperLimits.get(index, 0);
		...
		double brakeVelocity = DeadbandTools.applyDeadband(velocityDeadbandSizes[index].getDoubleValue(),
				joint.getQd());
		if (!Double.isInfinite(jointLimitLower) || !Double.isInfinite(velocityLimitLower)) {
			double qDotMinFromFD = (jointLimitLower - joint.getQ()) / controlDT;
			double qDotMin = MathTools.clamp(qDotMinFromFD, velocityLimitLower, velocityLimitUpper);
	...
	private void computeAccelerationLimitRestrictive(OneDoFJointBasics joint, double absoluteMaximumJointAcceleration,
			DMatrixRMaj qDDotMinToPack, DMatrixRMaj qDDotMaxToPack) {
		int index = jointIndexHandler.getOneDoFJointIndex(joint);
	...
	private void enforceJointTorqueLimit(OneDoFJointBasics joint, JointDesiredOutputBasics jointDesiredOutput,
			JointLimitData jointLimitData) {
		double torque;
		...
		if (!jointLimitData.hasPositionSoftLowerLimit() && joint.getQ() < jointLimitData.getPositionSoftLowerLimit()) {
			...
			if (jointLimitData.hasPositionLimitStiffness())
				stiffnessTorque = jointLimitData.getJointLimitStiffness()
						* (jointLimitData.getPositionSoftLowerLimit() - joint.getQ());
		...
		if (!jointLimitData.hasPositionSoftUpperLimit() && joint.getQ() > jointLimitData.getPositionSoftUpperLimit()) {
			...
			if (jointLimitData.hasPositionLimitStiffness())
				stiffnessTorque = jointLimitData.getJointLimitStiffness()
						* (jointLimitData.getPositionSoftUpperLimit() - joint.getQ());

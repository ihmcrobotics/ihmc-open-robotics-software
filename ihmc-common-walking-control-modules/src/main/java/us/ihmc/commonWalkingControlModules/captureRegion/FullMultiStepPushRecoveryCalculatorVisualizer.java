...
public class FullMultiStepPushRecoveryCalculatorVisualizer implements MultiStepPushRecoveryCalculatorVisualizer
{
   ...
   private final List<List<YoFrameConvexPolygon2D>> environmentalConstraintRegions = new ArrayList<>();

   private final int maxRegionDepth;

   public FullMultiStepPushRecoveryCalculatorVisualizer(String suffix, int maxRegionDepth, YoRegistry parentRegistry, YoGraphicsListRegistry graphicsListRegistry)
   {
      ...
      if (graphicsListRegistry != null)
      {
         ...
         for (int i = 0; i < maxRegionDepth; i++)
         {
            ...
            ArrayList<YoFrameConvexPolygon2D> constraintRegionList = new ArrayList<>();
            environmentalConstraintRegions.add(constraintRegionList);

            for (int j = 0; j < 10; j++)
            {

               String constraintRegionName = "environmentRegion" + i + "_" + j;
               YoFrameConvexPolygon2D constraintRegion = new YoFrameConvexPolygon2D(constraintRegionName, suffix, worldFrame, 30, registry);
               constraintRegionList.add(constraintRegion);

               YoArtifactPolygon environmentPolygonArtifact = new YoArtifactPolygon(constraintRegionName + suffix, constraintRegion, Color.DARK_GRAY, false);

               graphicsListRegistry.registerArtifact(listName, environmentPolygonArtifact);
            }
   ...
   public void reset()
   {
      for (int i = 0; i < maxRegionDepth; i++)
      {
         ...
         for (int j = 0; j < environmentalConstraintRegions.get(i).size(); j++)
            environmentalConstraintRegions.get(i).get(j).setToNaN();
      }
   }
   ...
   public void visualize(MultiStepRecoveryStepCalculator pushRecoveryCalculator)
   {
      ...
      for (int i = 0; i < depth; i++)
      {
         ...
         for (int j = 0; j < pushRecoveryCalculator.getNumberOfConstraintRegionsForStep(i); j++)
            environmentalConstraintRegions.get(i).get(j).set(pushRecoveryCalculator.getConstrainedReachableRegion(i, j));
      }
   }
}

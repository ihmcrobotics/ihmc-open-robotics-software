...
public class LinearMomentumRateControlModule implements SCS2YoGraphicHolder
{
   ...
   private final AlphaFilteredYoFrameVector3D filteredYoAngularMomentum;
   ...
   public void setInputFromWalkingStateMachine(LinearMomentumRateControlModuleInput input)
   {
      ...
      icpController.setUseHighCoPDamping(input.getIsCoPDamped());
   ...
   public boolean computeControllerCoreCommands()
   {
      ...
      boolean success = checkInputs(useAngularCapturePoint.getBooleanValue() ? angularCapturePoint : capturePoint,
                                    desiredCapturePoint,
      ...
      if (centerOfPressureCommandCalculator.getCenterOfPressureWeight() > 0.0)
      {
         centerOfPressureCommand.setDesiredCoP(desiredCoP);
         centerOfPressureCommand.setWeight(worldFrame,
                                           centerOfPressureCommandCalculator.getCenterOfPressureWeight(),
                                           centerOfPressureCommandCalculator.getCenterOfPressureWeight());
      }
   ...
   private void computeACP()
   {
      double mass = totalMassProvider.getValue();
      double desiredPendulumHeight = gravityZ / MathTools.square(omega0);
      double wmh = omega0 * mass * desiredPendulumHeight;

      angularCapturePoint.set(capturePoint);

      FrameVector3DReadOnly angularMomentum = angularVelocityCalculator.getAngularMomentum();
      yoAngularMomentum.set(angularMomentum);

      filteredYoAngularMomentum.update();

      angularCapturePoint.addX(1.0 / wmh * filteredYoAngularMomentum.getY());
      angularCapturePoint.addY(-1.0 / wmh * filteredYoAngularMomentum.getX());
   }
   ...
   private void computeICPController()
   {
      ...
      if (perfectCoP.containsNaN())
      {
         ...
         icpController.compute(supportPolygonInWorld,
                               ...
                               perfectCMP,
                               useAngularCapturePoint.getBooleanValue() ? angularCapturePoint : capturePoint,
                               centerOfMass2d,
                               omega0);
      }
      else
   ...
   {
      FramePoint2DReadOnly capturePointToUse = useAngularCapturePoint.getBooleanValue() ? angularCapturePoint : capturePoint;
      if (desiredCMP.containsNaN())
      {
         ...
         desiredCMP.set(capturePointToUse);

...
public class SmartCMPProjectorTwo extends CMPProjector
{
         ...
         FramePoint2D finalDesiredCapturePoint, FramePoint2D desiredCMP)
   {
      ...
      tempBoundingBox.set(supportPolygon.getBoundingBox());
      double diagonalLengthSquared = tempBoundingBox.getDiagonalLengthSquared();

      if (diagonalLengthSquared < 0.01 * 0.01)
      {
         desiredCMP.setIncludingFrame(supportPolygon.getCentroid());
         return;
      }
      ...
      if (finalDesiredCapturePoint != null)
      {
         ...
         FramePoint2DBasics[] finalDesiredICPToICPIntersections = supportPolygon.intersectionWith(rayFromICPAwayFromFinalDesiredICP);
         ...
         if (finalDesiredICPToICPIntersections != null && finalDesiredICPToICPIntersections.length > 1)
         {
            ...
            FramePoint2DBasics closestIntersection = findClosestIntersection(capturePoint, finalDesiredICPToICPIntersections);
            desiredCMP.set(closestIntersection);
            return;
         }

         cmpProjectedToVertex.set(true);
         boolean success = supportPolygon.getClosestPointWithRay(rayFromICPAwayFromFinalDesiredICP, desiredCMP);
         if (!success)
   ...
   private FramePoint2DBasics findClosestIntersection(FramePoint2D closestToPoint, FramePoint2DBasics... finalDesiredICPToICPIntersections)
   {
      FramePoint2DBasics closestIntersection = null;
      double closestDistanceSquared = Double.POSITIVE_INFINITY;
      for (FramePoint2DBasics framePoint2d : finalDesiredICPToICPIntersections)
      {

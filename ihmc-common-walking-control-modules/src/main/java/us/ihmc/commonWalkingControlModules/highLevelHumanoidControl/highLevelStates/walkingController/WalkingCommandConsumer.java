...
import controller_msgs.ManipulationAbortedStatus;
...
public class WalkingCommandConsumer
{
   ...
   public void consumeChestCommands(WalkingState currentState, boolean allowMotionRegardlessOfState)
   {
      if (commandConsumerWithDelayBuffers.isNewCommandAvailable(ChestHybridJointspaceTaskspaceTrajectoryCommand.class))
      {
         ...
         SO3TrajectoryControllerCommand so3Trajectory = command.getTaskspaceTrajectoryCommand();
         so3Trajectory.setSequenceId(command.getSequenceId());
   ...
   public void consumeGoHomeMessages()
   {
      ...
      for (int i = 0; i < commands.size(); i++)
      {
         ...
         if (command.getRequest(HumanoidBodyPart.HEAD))
         {
            headManager.goHome(command.getTrajectoryTime());
         }
      }
   }

...
namespace promp {
    ...
    public:
         ...
         */
        PROMPCALL ProMP(const std::vector<Trajectory>& data, int num_bf, double std_bf = -1)
            : _num_bf(num_bf),
              _alpha(1.0)
        {
            init(data, std_bf);
         ...
         *	\param std_bf	 standard deviation; this is set automatically to  1.0 / (n_rbf*n_rbf) if std_bf <= 0
         */
        PROMPCALL ProMP(const TrajectoryGroup& data, int num_bf, double std_bf = -1)
            : _num_bf(num_bf),
              _alpha(1.0)
        {
         ...
         */
        PROMPCALL ProMP(const Eigen::VectorXd& w, const Eigen::MatrixXd& cov_w, double std_bf, int n_sample, size_t dims, double time_mod = 1.0);
         ...
         */
        const Eigen::MatrixXd& PROMPCALL get_basis_function() const;

        /** \brief	generates basis functions
         *	\param phase
         *	\return matrix of basis functions
         */
        Eigen::MatrixXd PROMPCALL generate_basis_function(const Eigen::VectorXd& phase) const;

        /** \brief	maps time vector into a phase vector give a desired number of timesteps
         */
        Eigen::VectorXd PROMPCALL compute_phase(size_t timesteps) const;
         ...
         */
        const Eigen::VectorXd& PROMPCALL get_phase() const;
         ...
         * @return     The weights.
         */
        const Eigen::VectorXd& PROMPCALL get_weights() const;
         ...
         */
        const Eigen::MatrixXd& PROMPCALL get_covariance() const;
         ...
         */
        int PROMPCALL get_n_samples() const;
         ...
         */
        size_t PROMPCALL get_dims() const;
        ...
        */
         ...
         */
        Eigen::MatrixXd PROMPCALL generate_trajectory() const;
         ...
         * @return     Mean trajectory
         */
        Eigen::MatrixXd PROMPCALL generate_trajectory(size_t req_num_steps) const;
         ...
         */
        Eigen::MatrixXd PROMPCALL generate_trajectory_with_speed(double req_phase_speed) const;

        Eigen::MatrixXd PROMPCALL generate_trajectory_at(const Eigen::VectorXd& phase) const;
         ...
         *	\return
         */
        void PROMPCALL condition_goal(const Eigen::VectorXd& goal, const Eigen::MatrixXd& std);
         ...
         *	\return
         */
        void PROMPCALL condition_start(const Eigen::VectorXd& start, const Eigen::MatrixXd& std);
         ...
         */
        Eigen::MatrixXd PROMPCALL gen_traj_std_dev(size_t req_num_steps = 0) const;
         ...
         */
        std::vector<Eigen::MatrixXd> PROMPCALL generate_trajectory_covariance(size_t req_num_steps = 0) const;

        /**
         * @brief      Conditions all via points registered in 'viaPoints_'.
         * It updates  \a _mean_w and _cov_w  and clear _via_points vector
         */
        void PROMPCALL condition_via_points(const std::vector<std::tuple<int, Eigen::VectorXd, Eigen::MatrixXd>>& via_points);
         ...
         */
        void PROMPCALL condition_via_point(int t, const Eigen::VectorXd& via_point, const Eigen::MatrixXd& std);

        Eigen::VectorXd PROMPCALL get_upper_weights(double K);

        Eigen::VectorXd PROMPCALL get_lower_weights(double K);
         ...
         */
        inline double PROMPCALL phase_speed_from_steps(int steps) { return static_cast<double>(_s) / steps; }

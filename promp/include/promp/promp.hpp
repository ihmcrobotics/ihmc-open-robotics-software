...
namespace promp {
    ...
    public:
         ...
         */
        PROMPEXPORT ProMP(const Eigen::VectorXd& w, const Eigen::MatrixXd& cov_w, double std_bf, int n_sample, size_t dims, double time_mod = 1.0);
         ...
         */
        PROMPEXPORT const Eigen::MatrixXd& get_basis_function() const;

        /** \brief	generates basis functions
         *	\param phase
         *	\return matrix of basis functions
         */
        PROMPEXPORT Eigen::MatrixXd generate_basis_function(const Eigen::VectorXd& phase) const;

        /** \brief	maps time vector into a phase vector give a desired number of timesteps
         */
        PROMPEXPORT Eigen::VectorXd compute_phase(size_t timesteps) const;
         ...
         */
        PROMPEXPORT const Eigen::VectorXd& get_phase() const;
         ...
         * @return     The weights.
         */
        PROMPEXPORT const Eigen::VectorXd& get_weights() const;
         ...
         */
        PROMPEXPORT const Eigen::MatrixXd& get_covariance() const;
         ...
         */
        PROMPEXPORT int get_n_samples() const;
         ...
         */
        PROMPEXPORT size_t get_dims() const;
         ...
         */
        PROMPEXPORT Eigen::MatrixXd generate_trajectory() const;
         ...
         * @return     Mean trajectory
         */
        PROMPEXPORT Eigen::MatrixXd generate_trajectory(size_t req_num_steps) const;
         ...
         */
        PROMPEXPORT Eigen::MatrixXd generate_trajectory_with_speed(double req_phase_speed) const;

        PROMPEXPORT Eigen::MatrixXd generate_trajectory_at(const Eigen::VectorXd& phase) const;
         ...
         *	\return
         */
        PROMPEXPORT void condition_goal(const Eigen::VectorXd& goal, const Eigen::MatrixXd& std);
         ...
         *	\return
         */
        PROMPEXPORT void condition_start(const Eigen::VectorXd& start, const Eigen::MatrixXd& std);
         ...
         */
        PROMPEXPORT Eigen::MatrixXd gen_traj_std_dev(size_t req_num_steps = 0) const;
         ...
         */
        PROMPEXPORT std::vector<Eigen::MatrixXd> generate_trajectory_covariance(size_t req_num_steps = 0) const;

        /**
         * @brief      Conditions all via points registered in 'viaPoints_'.
         * It updates  \a _mean_w and _cov_w  and clear _via_points vector
         */
        PROMPEXPORT void condition_via_points(const std::vector<std::tuple<int, Eigen::VectorXd, Eigen::MatrixXd>>& via_points);
         ...
         */
        PROMPEXPORT void condition_via_point(int t, const Eigen::VectorXd& via_point, const Eigen::MatrixXd& std);

        PROMPEXPORT Eigen::VectorXd get_upper_weights(double K);

        PROMPEXPORT Eigen::VectorXd get_lower_weights(double K);
         ...
         */
        PROMPEXPORT inline double phase_speed_from_steps(int steps) { return static_cast<double>(_s) / steps; }

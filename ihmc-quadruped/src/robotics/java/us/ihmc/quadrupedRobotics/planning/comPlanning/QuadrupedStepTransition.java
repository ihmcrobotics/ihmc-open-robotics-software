...
class QuadrupedStepTransition
{
   ...
   {
      if (transitionQuadrants.contains(transitionQuadrant))
   ...
   public void addTransition(QuadrupedStepTransition other)
   {
      ...
      for (int i = 0; i < other.transitionQuadrants.size(); i++)
      {
         ...
         if (transitionQuadrants.contains(addingQuadrant))
   ...
   {
      String name = "Number of transitions = " + transitionTypes.size() + " at time " + transitionTime + ".";
      for (int i = 0; i < transitionTypes.size(); i++)
      {
         name += "\nTransition " + transitionQuadrants.get(i) + " = " + transitionTypes.get(i) + " at " + transitionPositions.get(transitionQuadrants.get(i));
      }

...
public class HeightMapMessagePubSubType implements us.ihmc.pubsub.TopicDataType<perception_msgs.msg.dds.HeightMapMessage>
{
   ...
   {
   		return "8459ec531806be12c46e4f17c23dd003c0b0ac6deb2194eeeb7fe59b74bfbe47";
   }
   ...
   public static int getMaxCdrSerializedSize(int current_alignment)
   {
      ...
      current_alignment += 4 + us.ihmc.idl.CDR.alignment(current_alignment, 4);current_alignment += (180000 * 4) + us.ihmc.idl.CDR.alignment(current_alignment, 4);

      current_alignment += 4 + us.ihmc.idl.CDR.alignment(current_alignment, 4);current_alignment += (180000 * 4) + us.ihmc.idl.CDR.alignment(current_alignment, 4);

      current_alignment += 4 + us.ihmc.idl.CDR.alignment(current_alignment, 4);current_alignment += (180000 * 4) + us.ihmc.idl.CDR.alignment(current_alignment, 4);

      current_alignment += 4 + us.ihmc.idl.CDR.alignment(current_alignment, 4);for(int i0 = 0; i0 < 180000; ++i0)
      {
   ...
   public static void write(perception_msgs.msg.dds.HeightMapMessage data, us.ihmc.idl.CDR cdr)
   {
      ...
      if(data.getKeys().size() <= 180000)
      cdr.write_type_e(data.getKeys());else
          throw new RuntimeException("keys field exceeds the maximum length: %d > %d".formatted(data.getKeys().size(), 180000));

      if(data.getHeights().size() <= 180000)
      cdr.write_type_e(data.getHeights());else
          throw new RuntimeException("heights field exceeds the maximum length: %d > %d".formatted(data.getHeights().size(), 180000));

      if(data.getVariances().size() <= 180000)
      cdr.write_type_e(data.getVariances());else
          throw new RuntimeException("variances field exceeds the maximum length: %d > %d".formatted(data.getVariances().size(), 180000));

      if(data.getCentroids().size() <= 180000)
      cdr.write_type_e(data.getCentroids());else
          throw new RuntimeException("centroids field exceeds the maximum length: %d > %d".formatted(data.getCentroids().size(), 180000));

      if(data.getNormals().size() <= 180000)
      cdr.write_type_e(data.getNormals());else
          throw new RuntimeException("normals field exceeds the maximum length: %d > %d".formatted(data.getNormals().size(), 180000));

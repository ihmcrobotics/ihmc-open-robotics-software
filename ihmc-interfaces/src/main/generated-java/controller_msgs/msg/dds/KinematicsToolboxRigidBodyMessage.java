...
/**
 * This message is part of the IHMC hole-body inverse kinematics module. This message contains all
 * the information needed to configure the objectives/constraints to apply on a given end-effector
 * in the solver.
 */
      ...
      implements Settable<KinematicsToolboxRigidBodyMessage>, EpsilonComparable<KinematicsToolboxRigidBodyMessage>
{
    ...
    * RigidBody.getNameBasedHashCode() for the computation of a rigid-body hash code.
    */
    ...
    * defined, it represents the desired position endEffector.getBodyFixedFrame(). The data is
    * assumed to be expressed in world frame.
    */
   public us.ihmc.euclid.tuple3D.Point3D desired_position_in_world_;
   /**
    * This is the desired orientation of the control frame. I the control frame has not been
    * defined, it represents the desired orientation of endEffector.getBodyFixedFrame(). The data is
    * assumed to be expressed in world frame.
    */
    ...
    * endEffector.getBodyFixedFrame(). By default, the control frame is coincident to
    * endEffector.getBodyFixedFrame(). The control frame is rigidly attached to the end-effector.
    */
   public us.ihmc.euclid.tuple3D.Point3D control_frame_position_in_end_effector_;
   /**
    * This is the orientation of the control frame expressed in endEffector.getBodyFixedFrame(). By
    * default, the control frame is coincident to endEffector.getBodyFixedFrame().
    */
    ...
    * be controlled. The selection frames coming along with the given selection matrix are used to
    * determine to what reference frame the selected axes are referring to. For instance, if only
    * the hand height in world should be controlled on the linear z component of the selection
    * matrix should be selected and the reference frame should be world frame. When no reference
    * frame is provided with the selection matrix, it will be used as it is in the control frame,
    * i.e. the body-fixed frame if not defined otherwise.
    */
   ...
   /**
    * Weight matrix used to define the priority of controlling the translation around each axis on
    * the solver side.
    */

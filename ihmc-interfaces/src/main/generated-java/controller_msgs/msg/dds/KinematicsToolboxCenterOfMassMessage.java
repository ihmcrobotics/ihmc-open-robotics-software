...
/**
 * This message is part of the IHMC hole-body inverse kinematics module. It holds all the
 * information needed for detailing the type of constraint to apply to the center of mass.
 */
      ...
      implements Settable<KinematicsToolboxCenterOfMassMessage>, EpsilonComparable<KinematicsToolboxCenterOfMassMessage>
{
    ...
    * should be controlled. The selection frame coming along with the given selection matrix is used
    * to determine to what reference frame the selected axes are referring to. For instance, if only
    * the hand height in world should be controlled on the linear z component of the selection
    * matrix should be selected and the reference frame should world frame. When no reference frame
    * is provided with the selection matrix, it will be used as it is in the control frame, i.e. the
    * body-fixed frame if not defined otherwise.
    */
   public controller_msgs.msg.dds.SelectionMatrix3DMessage selection_matrix_;
   /**
    * Specifies the priority of controller the position along each axis independently. If no frame
    * is provided, the weight matrix will be applied in the center of mass frame which is aligned
    * with the world axes.
    */

...
/**
 * This message is part of the IHMC whole-body controller API. General message carrying the
 * information needed to safely queue messages.
 */
public class QueueableMessage extends Packet<QueueableMessage> implements Settable<QueueableMessage>, EpsilonComparable<QueueableMessage>
{
    ...
    * trajectory point. Otherwise the controller will prepend a first trajectory point at the
    * current desired position. When EXECUTION_MODE_QUEUE is chosen: - The message must carry the ID
    * of the message it should be queued to. - The very first message of a list of queued messages
    * has to be an EXECUTION_MODE_OVERRIDE message. - For trajectory messages: - the trajectory
    * point times are relative to the the last trajectory point time of the previous message. - the
    * time of the first trajectory point has to be greater than zero. - When joint-space trajectory:
    ...
    * lost on the way. If a message appears to be missing (previous_message_id different from the
    * last message_id received by the controller), the motion is aborted.
    */

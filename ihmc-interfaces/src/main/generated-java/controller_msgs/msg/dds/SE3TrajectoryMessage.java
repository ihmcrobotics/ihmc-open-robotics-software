 ...
 * to execute a trajectory in taskspace (position and orientation) by defining trajectory points. A
 * third order polynomial function is used to interpolate positions and a Hermite based curve (third
 * order) is used to interpolate the orientations. To execute a single straight line trajectory to
 * reach a desired pose, set only one trajectory point with zero velocity and its time to be equal
 * to the desired trajectory time.
 */
public class SE3TrajectoryMessage extends Packet<SE3TrajectoryMessage> implements Settable<SE3TrajectoryMessage>, EpsilonComparable<SE3TrajectoryMessage>
{
   ...
   /**
    * Pose of custom control frame expressed in the end-effector frame. This is the frame attached
    * to the rigid body that the taskspace trajectory is defined for.
    */
   ...
   {
      taskspace_trajectory_points_ = new us.ihmc.idl.IDLSequence.Object<controller_msgs.msg.dds.SE3TrajectoryPointMessage>(1,

...
Uses a local VLM (Qwen2.5-VL or compatible) to drive robot behavior in the IHMC explosive breaching demo.
Two backends are supported — **vLLM** (high-throughput GPU server) and **Ollama** (easy local setup).
...
│   ├── vlm_interface.py          # VLM client wrapper — vLLM backend
│   ├── ollama_interface.py       # VLM client wrapper — Ollama backend (drop-in replacement)
...
└── test/                     # Server startup scripts, notebooks, and test images
    ├── start_qwen_vl_server.sh         # Launch vLLM server — x86 dev machine
    ├── start_qwen_server.sh            # Launch vLLM text-only server — x86 dev machine
    ├── start_qwen_vl_server_jetson.sh  # Launch vLLM server — Jetson Orin (Docker)
    ├── RUN_GUIDE.md                    # Run instructions — x86 dev machine
    ├── JETSON_RUN_GUIDE.md             # Run instructions — Jetson Orin (JetPack 6.2)
...
Two interchangeable clients share an identical interface (`load_config`, `call_model`, `first_log_interaction`):

| Class | File | Backend | Default host |
|---|---|---|---|
| `VLMInterface` | `vlm_interface.py` | vLLM (OpenAI-compatible) | `http://localhost:8000/v1` |
| `OllamaInterface` | `ollama_interface.py` | Ollama native API | `http://localhost:11434` |

Each instance is configured by a JSON file that defines:
...
prompt assembled from config, and the full message is sent to the server.
...
```jsonc
{
    "api_base": "http://localhost:8000/v1", // vLLM backend URL (VLMInterface)
    "ollama_host": "http://localhost:11434",// Ollama host URL (OllamaInterface); omit to use default
    "model": "auto",            // "auto" picks the first model from the server's list
    "temperature": 0.0,
    "max_tokens": 512,
    "top_p": 1.0,
...
Both `api_base` and `ollama_host` can coexist in the same config file — each client reads only its own key.
...
| | vLLM | Ollama |
|---|---|---|
| Best for | High-throughput, production use | Quick local setup, experimentation |
| GPU required | Yes | No (CPU mode available, slower) |
| Install | `pip install vllm` | `curl -fsSL https://ollama.com/install.sh \| sh` |
| Model format | HuggingFace checkpoint | Ollama model library (`ollama pull`) |
| Vision support | Yes (with VL model) | Yes (with vision-capable model) |
| Client class | `VLMInterface` | `OllamaInterface` |
| Coordinator import | `from vlm_interface import VLMInterface` | `from ollama_interface import OllamaInterface as VLMInterface` |
...
pip install vllm openai ollama numpy scipy ipykernel
...
| Problem | Fix |
|---|---|
| `address already in use` on port 8000 | Another vLLM process is running — `pkill -f vllm` then retry |
| `CUDA out of memory` | Lower `--gpu-memory-utilization` (e.g. `0.70`) or close other GPU apps |
| `model not found` | Verify the model path in the script matches where `huggingface-cli` downloaded it |
| vLLM errors appear during `colcon build` | The `modules/ros2-env/COLCON_IGNORE` file is missing — `touch modules/ros2-env/COLCON_IGNORE` then rebuild |
...
See [`test/RUN_GUIDE.md`](test/RUN_GUIDE.md) for full x86 run instructions.
For Jetson Orin (JetPack 6.2), see [`test/JETSON_RUN_GUIDE.md`](test/JETSON_RUN_GUIDE.md).
...
`behavior_coordinator.py` imports `VLMInterface` by default (vLLM).
...
```bash
# Install Ollama (starts a systemd service automatically on Linux)
curl -fsSL https://ollama.com/install.sh | sh

# Pull a vision-capable model (choose one)
ollama pull qwen2.5vl        # Qwen2.5-VL 7B — closest to the vLLM setup
ollama pull llava             # LLaVA 7B — widely tested alternative
```

Ollama runs as a background service on `http://localhost:11434`. No manual server start needed after install.

**Switch the coordinator to use Ollama:**
...
`OllamaInterface` auto-detects the first available model via `ollama list`. To pin a specific model,
add `"model": "qwen2.5vl"` to the relevant config JSON.

**Troubleshooting Ollama:**

| Problem | Fix |
|---|---|
| `ConnectionError: Failed to connect to Ollama` | Run `systemctl status ollama` — if inactive, `sudo systemctl start ollama` |
| `"address already in use"` on `ollama serve` | The systemd service is already running; ignore the error |
| Slow responses | Ollama runs on CPU if no GPU is detected; expect 10-60s per call |
| Wrong model used | Set `"model": "<name>"` explicitly in config, or run `ollama list` to see available models |
| Ollama errors appear during `colcon build` | The `modules/ros2-env/COLCON_IGNORE` file is missing — `touch modules/ros2-env/COLCON_IGNORE` then rebuild |

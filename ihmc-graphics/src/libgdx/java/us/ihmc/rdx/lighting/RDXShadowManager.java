...
public class RDXShadowManager
{
   ...
   public RDXShadowManager(float antiAliasing, float ambientLight)
   {
      ...
      shadowSceneBatch = new ModelBatch(new DefaultShaderProvider()
      {
         ...
         {
            return new RDXShadowSceneShader(renderable, shadowSceneShaderProgram);
         }
      });
   ...
   {
      for (RDXPointLight additionalOffscreenLight : additionalOffscreenLights)
      {
         additionalOffscreenLight.render(renderableProviders);
      }
      for (RDXPointLight pointLight : pointLights)
      {
         pointLight.render(renderableProviders);
      }
      for (RDXDirectionalLight directionalLight : directionalLights)
      {

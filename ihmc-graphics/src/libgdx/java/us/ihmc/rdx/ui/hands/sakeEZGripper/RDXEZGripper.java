...
import us.ihmc.sakeros2.EZGripperError;
import us.ihmc.sakeros2.EZGripperManager;
...
public class RDXEZGripper implements RDXHandInterface
{
   ...
   {
      return communication.readState(handSide).getIsCalibrated();
   }

   @Override
   public boolean needsReset()
   {
      return communication.readState(handSide).getErrorCode() != EZGripperError.NONE.errorCode;
   }
   ...
   public void renderImGuiWidgets()
   {
      ...
      for (SakeHandPreset preset : presetButtons)
      {
         if (ImGui.button(labels.get(preset.getPascalCasedName())))
         {
            ...
            // Set the command
            communication.getCommand(handSide).setOperationMode(EZGripperCommand.POSITION_CONTROL);
            communication.getCommand(handSide).setGoalPosition((float) normalizedPosition);
            communication.getCommand(handSide).setMaxEffort((float) normalizedEffort);
            communication.getCommand(handSide).setTorqueOn(normalizedEffort > 1E-3);
      ...
      {
         communication.getCommand(handSide).setOperationMode(EZGripperCommand.CALIBRATION);
         commandChanged.set();
      }
      ...
      {
         communication.getCommand(handSide).setOperationMode(EZGripperCommand.ERROR_RESET);
         commandChanged.set();
      }
      ...
      communication.getCommand(handSide).setTemperatureLimit((byte) (imAutoCoolDownEnabled.get() ? temperatureLimit.get() : 255));
   ...
   public void sendCommand(HandAction action)
   {
      switch (action)
      {
         case OPEN ->
         {
            EZGripperCommand command = communication.getCommand(handSide);

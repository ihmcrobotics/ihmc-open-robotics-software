...
public class ComplexNumberTest
{
   ...
   public void testCommonUsage()
   {
      ...
      assertEquals(imag, complexNumber.imaginary(), epsilon);
   ...
   public void testZero()
   {
      ...
      assertEquals(0.0, complexNumber.magnitude());
      assertEquals(0.0, complexNumber.magnitudeSquared());
   }
   ...
   public void testALotOfRandomComplexNumbers()
   {
      ...
      for (int i = 0; i < numberOfTests; i++)
      {
         ...
         assertTrue(complexNumber.epsilonEquals(complexNumber.conjugate().conjugate(), epsilon));
   ...
   public void regressionTest()
   {
      ...
      ComplexNumber conj = complexNumber.conjugate();
   ...
   public void testInitialization()
   {
      ...
      assert (c1.real() == 0.0);
      assert (c1.imaginary() == 0.0);
      c1 = new ComplexNumber(1, 1);
      assert (c1.real() == 1.0);
      assert (c1.imaginary() == 1.0);
      c1.set(2.12, 3.14159);
      assert (c1.real() == 2.12);
      assert (c1.imaginary() == 3.14159);
      c1.setFromPolar(2, Math.toRadians(90));
      assert (Math.abs(c1.real()) < Epsilons.ONE_BILLIONTH);
      assert (c1.imaginary() == 2);
   }
   ...
   public void testMath()
   {
      ...
      assert (c3.imaginary() == c1.imaginary() + c2.imaginary());
      c3.times(c1, c2);
      assert (c3.real() == 2.4 * 1.2 - 4.5 * 3.1);
      assert (c3.imaginary() == 2.4 * 4.5 + 1.2 * 3.1);
      c3.setToPurelyReal(1);
      assert (c3.magnitude() == 1);
      assert (c3.angle() == 0);
      c3.getRoots(c1, c2);
      assert (c1.real() == 1);
      assert (c1.imaginary() == 0);
      assert (c2.real() == -1);
      assert (Math.abs(c2.imaginary()) < Epsilons.ONE_BILLIONTH);
      c2.getRoots(c1, c3);
      assert (Math.abs(c1.real() - 0) < Epsilons.ONE_BILLIONTH);
      assert (Math.abs(c1.imaginary() - 1) < Epsilons.ONE_BILLIONTH);
      assert (Math.abs(c3.real() - 0) < Epsilons.ONE_BILLIONTH);
      assert (Math.abs(c3.imaginary() + 1) < Epsilons.ONE_BILLIONTH);
   }
   ...
   public void tesNthRoots()
   {
      ...
      for (int i = 0; i < roots.size(); i++)
      {
         ...
         assert (MathTools.epsilonCompare(roots.get(i).imaginary(), Math.sin(i * 2.0 * Math.PI / 8.0), Epsilons.ONE_BILLIONTH));
      }
   }
}

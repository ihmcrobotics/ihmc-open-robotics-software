   ...
   public void testRandomProblemsAgainstTrueLeastSquaresProjection()
   {
      ...
      for (int i = 0; i < 1000; i++)
      {
         ...
         for (int j = 0; j < projectorExpected.getNumRows(); j++)
         {
            ...
            {
               assertEquals(projectorExpected.get(j, k), projector.get(j, k), 1e-7, "Iteration " + i + " failed on index " + " (" + j + ", " + k + ").");
            }
         }
         ...
         for (int j = 0; j < projectedMatrixExpected.getNumRows(); j++)
         {
            ...
            {
               assertEquals(projectedMatrixExpected.get(j, k),
                            projectedMatrix.get(j, k),
                            1e-7,
                            "Iteration " + i + " failed on index " + " (" + j + ", " + k + ").");
               assertEquals(projectedMatrixExpected.get(j, k),
                            matrixToProject.get(j, k),
                            1e-7,
                            "Iteration " + i + " failed on index " + " (" + j + ", " + k + ").");
            }
         }

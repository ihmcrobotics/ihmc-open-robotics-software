...
public class ComplexConjugateMode
{
   ...
   public TransferFunctionMatrix constructTransferFunctionMatrix()
   {
      ...
      Polynomial denominatorPolynomial = new Polynomial( eigenvalue.magnitudeSquared(), -2.0 * eigenvalue.real(), 1.0);
      ...
      for (int i = 0; i < order; i++)
      {
         for (int j = 0; j < order; j++)
         {
            ...
            double[] numeratorCoefficients = new double[] {-2.0 * eigenvalue.real() * Rij.real() - 2.0 * eigenvalue.imag() * Rij.imag(), 2.0 * Rij.real() };

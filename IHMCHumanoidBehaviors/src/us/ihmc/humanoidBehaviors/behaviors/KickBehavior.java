...
public class KickBehavior extends BehaviorInterface
{
   ...
   public KickBehavior(OutgoingCommunicationBridgeInterface outgoingCommunicationBridge, DoubleYoVariable yoTime, BooleanYoVariable yoDoubleSupport,SDFFullHumanoidRobotModel fullRobotModel,
   ...
   public void doControl()
   {
      ...
      {
//         setObjectToKickPoint(new FramePoint(ReferenceFrame.getWorldFrame(),0,0,0));
         checkInput();
      }
      else
      {
   ...
   private void setupPipeline()
   {
      ...
      submitFootPosition(kickFoot, new FramePoint(ankleZUpFrames.get(kickFoot.getOppositeSide()), -0.3, kickFoot.negateIfRightSide(0.25), 0.127));

      submitFootPosition(kickFoot, new FramePoint(objectToKickPose.getReferenceFrame(), objectToKickPose.getX(), objectToKickPose.getY(), 0.127));

      submitFootPosition(kickFoot, new FramePoint(ankleZUpFrames.get(kickFoot.getOppositeSide()), 0.1, kickFoot.negateIfRightSide(0.25), 0.127));
   ...
   private void submitFootPose(RobotSide robotSide, FramePose desiredFootPose)
   {
      ...
      FootPoseTask footPoseTask = new FootPoseTask(robotSide, desiredFootPosition, desiredFootOrientation, yoTime, footPoseBehavior,
            trajectoryTime.getDoubleValue(), 0.0);

...
public class DoublePendulumRobot extends RobotWithClosedFormDynamics
{
   ...
   private final double length1 = 0.7, length2 = 0.4;
   private final double Ixx1 = 0.4, Ixx2 = 0.5;
   private final double damping1 = 0.1, damping2 = 0.2;
   ...
   public DoublePendulumRobot(String name, double initialQFirstJoint, double initialQdFirstJoint, double initialQSecondJoint, double initialQdSecondJoint)
   {
      ...
      link1.setMass(mass1);
      link1.setMomentOfInertia(Ixx1, 0.0, 0.0);
      link1.setComOffset(0.0, 0.0, -0.5 * length1);
      ...
      link2.setMass(mass2);
      link2.setMomentOfInertia(Ixx2, 0.0, 0.0);
      link2.setComOffset(0.0, 0.0, -0.5 * length2);
   ...
   public void assertStateIsCloseToLagrangianCalculation(double epsilon)
   {
      ...
      double H00 = Ixx1 + Ixx2 + mass2 * MathTools.square(length2) + 2.0 * mass2 * length1 * (0.5 * length2) * Math.cos(q2);
      double H01 = Ixx2 + mass2 * length1 * (0.5 * length2) * Math.cos(q2);
      double H10 = H01;
      double H11 = Ixx2;

      double C00 = -2.0 * mass2 * length1 * (0.5 * length2) * Math.sin(q2) * qd2;
      double C01 = - mass2 * length1 * (0.5 * length2) * Math.sin(q2) * qd2;
      double C10 = - mass2 * length1 * (0.5 * length2) * Math.sin(q2) * qd1;
      double C11 = 0.0;

      double G00 = (mass1 * (0.5 * length1 + mass2 * length1)) * gravityZ.getDoubleValue() * Math.sin(q1)  + mass2 * gravityZ.getDoubleValue() * length2 * Math.sin(q1 + q2);
      double G10 = mass2 * gravityZ.getDoubleValue() * length2 * Math.sin(q1 + q2);
      ...
      if(Math.abs(qdd.get(0, 0) - qdd1) > epsilon || Math.abs(qdd.get(0, 1) - qdd2) > epsilon)
      {

...
public class ThorContactPointParameters extends RobotContactPointParameters
{
   private final SideDependentList<ArrayList<Point2D>> footGroundContactPoints = new SideDependentList<>();

   private final DRCRobotJointMap jointMap;

   public ThorContactPointParameters(DRCRobotJointMap jointMap)
   {
      super(jointMap, ThorPhysicalProperties.footWidthForControl, ThorPhysicalProperties.footLengthForControl, ThorPhysicalProperties.soleToAnkleFrameTransforms);
      ...
      for (Collision collision : link.getCollisions())
      {
         ...
         if (name.contains("ctrl_contact"))
         {
            ...
            for (RobotSide robotSide : RobotSide.values)
            {
               ...
               {
                  footGroundContactPoints.get(robotSide).add(projectOnPlane(ThorPhysicalProperties.getSoleToAnkleFrameTransform(robotSide), gcOffset));
                  assigned = true;
                  break;
               }

...
/**
 * Base class for constructing octrees. Contains internal functions for managing data structures.
 * All data structures are managed by this tree and can be reclaimed by calling {@link #reset}.
 *
 * @author Peter Abeles
 */
...
public abstract class ConstructOctree<O extends Octree, P extends GeoTuple>
{
   ...
   protected FastQueue<Octree.Info<P>> storageInfo = new FastQueue<Octree_F64.Info<P>>(Octree.Info::new);
   ...
   public ConstructOctree(Class<O> octreeType)
   {
      ...
      storageNodes = new FastQueue<O>(() ->
      {
         try
         {
            return octreeType.newInstance();
         }
   ...
   /**
    * Discards the existing tree structure and recycles its data. All reference to external data not
    * owned by the graph is discarded
    */
   public void reset()
   {
   ...
   /**
    * Checks to see if the child already exists. If not it creates the child. Info is added to the
    * child's points.
    */
   ...
   /**
    * Checks to see if the child already exists. If not it creates the child. If the child won't occupy
    * a physically possible space then null is returned.
    */
   ...
   /**
    * Checks to see if the provided cube has a non-zero positive volume. If the divisor is the same as
    * one of the corners then physically impossible spaces can be constructed.
    */

...
public class AI2RNodeExecutor extends BehaviorTreeNodeExecutor<AI2RNodeState, AI2RNodeDefinition>
{
      ...
      syncedRobot.addRobotConfigurationDataReceivedCallback(() ->
      {
         if (ros2LogRecord != null)
         {
            ...
            for (float jointAngle : nonFingerValues)
               kinematicsStatus.getDesiredJointAngles().add(jointAngle);
            for (int i = 0; i < nonFingerValues.length; i++)
            ...
            for (float jointVelocity : nonFingerValues)
               kinematicsStatus.getDesiredJointVelocities().add(jointVelocity);
   ...
   private static double[] generateRandomJointPositions(Random random, RobotSide side, ArmJointName[] armJointNames, OneDoFJointBasics[] armJoints)
   {
      ...
      for (int i = 0; i < armJoints.length; i++)
      {
         ...
         {
            LogTools.warn("Invalid randomization bounds for {}. Falling back to joint limits.", joint.getName());
               ...
               if (checkpoint.getLeafIndex() < leaf.getLeafIndex())
               {
                  ...
                  if (leaf instanceof ConditionNodeState conditionNodeState)
                  {
                     ...
                     if (conditionNodeState.getDefinition().getConditionType().getValue() == ConditionNodeType.PROXIMITY)
                     {
                        ...
                        failureMessage.getPositionError().set(new Point3D(error, 0.0, 0.0));

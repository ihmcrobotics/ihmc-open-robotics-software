      ...
      if (state.arePullRetryNodesPresent())
      {
         ...
         if (!waitForPullScrewToFinish && state.getPostPullDoorEvaluationAction().getIsExecuting())
         {
            if (yoloDoorHandleNode != null)
            {
               ...
               double distanceHandleFromStart = yoloDoorHandleNode.getNodeToParentFrameTransform()
                                                                  .getTranslation()
               ...
               if (state.getDoorHandleDistanceFromStart().getValue() < openedDoorHandleDistanceFromStart)
               {
                  state.getLogger().info("""
                                               ...
                                               Going back to %s.
                                               """.formatted(state.getDoorHandleDistanceFromStart().getValue(),
                                                             openedDoorHandleDistanceFromStart,
...
//            }
         ...
         if (!waitForGraspToFinish && state.getPostGraspEvaluationAction().getIsExecuting())
         {
            if (staticHandleClosedDoor != null)
            {
               double handToHandleDistance = syncedRobot.getFullRobotModel()
                                                        .getHandControlFrame(RobotSide.RIGHT)
                                                        .getTransformToDesiredFrame(staticHandleClosedDoor.getNodeFrame())
                                                        .getTranslation()
                                                        .norm();

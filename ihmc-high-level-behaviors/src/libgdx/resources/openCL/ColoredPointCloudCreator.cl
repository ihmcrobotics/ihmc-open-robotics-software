// TODO: float vs double in here? pick one, probably double for all the
// parameters
                             ...
                             global float *parameters)
{
   ...
   bool sinusoidal =
       parameters[23] >
       0.5; // TODO: Create a boolean OpenCL buffer for these binary parameters
   ...
   // Flip because positive yaw is to the left, but image coordinates go to the
   // right
   ...
   if (viewMode == COLOR)
   {
      ...
      {
         uint4 rgba8888Color =
             read_imageui(colorRGBAImage, (int2)(pixelIndexX, pixelIndexY));
         color = (rgba8888Color.x << 24) | (rgba8888Color.y << 16) |
                 (rgba8888Color.z << 8) | 255;
      }
      else
      {
   ...
   {
      //              float4 rgb =
      //              calculateGradientColorFromDepth(eyeDepthInMeters); color =
      //              ((int)rgb.x << 24) | ((int)rgb.y << 16) | ((int)rgb.z <<
      //              8) | 255;
   ...
   else
   {
      ...
      //               color = ((int)rgb.x << 24) | ((int)rgb.y << 16) |
      //               ((int)rgb.z << 8) | 255; printf("r: %f, g: %f, b: %f\n",
      //               rgb.x,rgb.y,rgb.z);
      color = calculateGradientColor((double)worldFramePoint.z, sinusoidal);
   }

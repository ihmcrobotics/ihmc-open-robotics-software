 ...
 */
public class RDXSphericalDepthSensorSimulator
{
   ...
   private ArrayList<RDXFrustumVisualizer> frustumVisualizers;
   ...
   public RDXSphericalDepthSensorSimulator(String sensorName,
                                           double fieldOfViewY,
                                           int imageWidth,
                                           int imageHeight,
                                           double minRange,
                                           ...
                                           boolean simulateL515Noise)
   {
      ...
      this.updatePeriod = UnitConversions.hertzToSeconds(10.0);
   }
   ...
   public void create(FloatBuffer pointCloudRenderingBufferToPack)
   {
      ...
      for (int i = 0; i<4; i++)
      {
         PerspectiveCamera camera = new PerspectiveCamera(fieldOfViewY.get(), imageWidth, imageHeight);
      ...
      frustumVisualizers = new ArrayList<>();
      frustumVisualizers.add(new RDXFrustumVisualizer());
      frustumVisualizers.add(new RDXFrustumVisualizer());
      frustumVisualizers.add(new RDXFrustumVisualizer());
      frustumVisualizers.add(new RDXFrustumVisualizer());
   }
   ...
   public void render(RDX3DScene scene, boolean colorBasedOnWorldZ, Color userPointColor, float pointSize)
   {
      ...
      if (renderFrustum.get())
      {
         for(int i = 0; i<4; i++)
         {
            frustumVisualizers.get(i).generateMesh(cameras.get(i).frustum);
            frustumVisualizers.get(i).update();
         }
      }
      modelBatch.begin(cameras.get(0));
      ...
      parametersBuffer.getBytedecoFloatBufferPointer().put(0, cameras.get(0).near);
      parametersBuffer.getBytedecoFloatBufferPointer().put(1, cameras.get(0).far);
   ...
   {
      if (renderFrustum.get())
         frustumVisualizers.get(0).getRenderables(renderables, pool);
   }
   ...
   {
      cameras.get(0).position.setZero();
      cameras.get(0).up.set(0.0f, 0.0f, 1.0f);
      cameras.get(0).direction.set(1.0f, 0.0f, 0.0f);
      cameras.get(0).transform(worldTransform);

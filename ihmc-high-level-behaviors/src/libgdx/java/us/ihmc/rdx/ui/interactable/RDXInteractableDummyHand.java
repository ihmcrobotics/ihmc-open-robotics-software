...
public class RDXInteractableDummyHand implements RenderableProvider
{
   ...
   private RigidBodyTransform[] fingersTransforms;
   ...
   {
      this.referenceFrameHand = ReferenceFrameMissingTools.constructFrameWithChangingTransformToParent(ReferenceFrame.getWorldFrame(), transformToParentToModify);
      ...
      this.fingersTransforms = FINGERS_TO_PALM_NEUTRAL;
   ...
   private void updateFingers(ImGui3DViewInput input)
   {
      LibGDXTools.toLibGDX(fingersFrames[0].getTransformToRoot(), fingersModelInstances[0].transform);
      LibGDXTools.toLibGDX(fingersFrames[1].getTransformToRoot(), fingersModelInstances[1].transform);
   }

   public void closeGripper()
   {
      fingersTransforms = FINGERS_TO_PALM_CLOSE;
   }

   public void openGripper()
   {
      fingersTransforms = FINGERS_TO_PALM_OPEN;
   }

   public void setGripperToNeutral()
   {
      fingersTransforms = FINGERS_TO_PALM_NEUTRAL;
   }

   public void setGripperClosure(double closure)
   {
      EuclidCoreMissingTools.setYawPitchRollDegrees(fingersTransforms[0].getRotation(), closure, fingersTransforms[0].getRotation().getPitch(), fingersTransforms[0].getRotation().getRoll());
      EuclidCoreMissingTools.setYawPitchRollDegrees(fingersTransforms[1].getRotation(), -closure, fingersTransforms[1].getRotation().getPitch(), fingersTransforms[1].getRotation().getRoll());
   }
   ...
   private double calculateClosestCollision(Line3DReadOnly mousePickRay)
   {
      ...
      intersectionHandOrigin.getTranslation().set(new Vector3D(interactableHandFrameModel.getReferenceFrame().getTransformToWorldFrame().getTranslationX() + 0.05, intersectionHandOrigin.getTranslationY(), intersectionHandOrigin.getTranslationZ()));

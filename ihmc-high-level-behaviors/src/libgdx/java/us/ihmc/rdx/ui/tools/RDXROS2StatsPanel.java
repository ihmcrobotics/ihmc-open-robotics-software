...
public class RDXROS2StatsPanel extends RDXPanel
{
   ...
   private void renderImGuiWidgets()
   {
      ...
      {
         ImGui.text("%s %s %s %d %d %d".formatted(publisherStats.getPublisher().getAttributes().getHumanReadableTopicName(),
                                                  publisherStats.getPublisher().getAttributes().getHumanReadableTopicDataTypeName(),
      ...
      {
         ImGui.text("%s %s %s %d".formatted(subscriberStats.getSubscriber().getAttributes().getHumanReadableTopicName(),
                                            subscriberStats.getSubscriber().getAttributes().getHumanReadableTopicDataTypeName(),
                                            subscriberStats.getSubscriber().getAttributes().getReliabilityKind().name(),
                                            subscriberStats.getNumberOfReceivedMessages()));
      }
   }
}

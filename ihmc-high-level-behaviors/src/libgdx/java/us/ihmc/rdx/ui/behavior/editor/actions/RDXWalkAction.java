...
public class RDXWalkAction extends WalkActionState implements RDXBehaviorAction
{
   ...
   private final RDXSelectablePathControlRingGizmo footstepPlannerGoalGizmo = new RDXSelectablePathControlRingGizmo(getConditionalReferenceFrame().get(),
   ...
   private final ImDoubleWrapper swingDurationWidget = new ImDoubleWrapper(this::getSwingDuration,
                                                                           this::setSwingDuration,
                                                                           imDouble -> ImGui.inputDouble(labels.get("Swing duration"), imDouble));
   private final ImDoubleWrapper transferDurationWidget = new ImDoubleWrapper(this::getTransferDuration,
                                                                              this::setTransferDuration,
                        ...
                        ReferenceFrameLibrary referenceFrameLibrary)
   {
      ...
      for (RobotSide side : RobotSide.values)
      {
         ...
         RDXPose3DGizmo footGizmo = new RDXPose3DGizmo(getGoalFootstepToParentTransforms().get(side), getConditionalReferenceFrame().get());
         ...
         goalFootPose.setToZero(getConditionalReferenceFrame().get());
   ...
   public void updateAfterLoading()
   {
      ...
      footstepPlannerGoalGizmo.getPathControlRingGizmo().setGizmoFrame(getConditionalReferenceFrame().get());
      for (RobotSide side : RobotSide.values)
      {
         goalFeetGizmos.get(side).setParentFrame(getConditionalReferenceFrame().get());
      }
   }
   ...
   public void update(boolean concurrentActionIsNextForExecution)
   {
      ...
      if (footstepPlannerGoalGizmo.getPathControlRingGizmo().getGizmoFrame() != getConditionalReferenceFrame().get())
      {
         footstepPlannerGoalGizmo.getPathControlRingGizmo().setGizmoFrame(getConditionalReferenceFrame().get());
         for (RobotSide side : RobotSide.values)
         {

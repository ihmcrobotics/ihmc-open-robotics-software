...
public class RDXROS1VideoVisualizer extends RDXOpenCVVideoVisualizer implements RDXROS1VisualizerInterface
{
   ...
   public RDXROS1VideoVisualizer(String title, String topic)
   {
      this(title, topic, false);
   }

   public RDXROS1VideoVisualizer(String title, String topic, boolean flipY)
   {
   ...
   private void processIncomingMessageOnThread(Image image)
   {
      ...
      if (videoProcessor == null)
      {
         ...
         {
            videoProcessor = new RDXROS1GrayscaleVideoProcessing();
         }
         else if (cvType == opencv_core.CV_8UC4)
         {
            videoProcessor = new RDXROS1RGBA8VideoProcessing();
         }
         else
         {

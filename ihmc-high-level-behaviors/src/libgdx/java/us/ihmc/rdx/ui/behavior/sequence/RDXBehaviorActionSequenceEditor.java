...
public class RDXBehaviorActionSequenceEditor
{
                      ...
                      ReferenceFrameLibrary referenceFrameLibrary)
   {
      ...
      executionNextIndexStatusSubscription = ros2ControllerHelper.subscribe(ROS2BehaviorActionSequence.EXECUTION_NEXT_INDEX_STATUS_TOPIC);
      automaticExecutionStatusSubscription = ros2ControllerHelper.subscribe(ROS2BehaviorActionSequence.AUTOMATIC_EXECUTION_STATUS_TOPIC);
      executionNextIndexRejectionSubscription = ros2ControllerHelper.subscribe(ROS2BehaviorActionSequence.EXECUTION_NEXT_INDEX_REJECTION_TOPIC);
      sequenceStatusSubscription = ros2ControllerHelper.subscribe(ROS2BehaviorActionSequence.SEQUENCE_STATUS_TOPIC);
      sequenceStatusSubscription.addCallback(message -> ++receivedSequenceStatusMessageCount);
      executionStatusSubscription = ros2ControllerHelper.subscribe(ROS2BehaviorActionSequence.ACTIONS_EXECUTION_STATUS);
   }
   ...
   private void commandNextActionIndex(int nextActionIndex)
   {
      ...
      ros2ControllerHelper.publish(ROS2BehaviorActionSequence.EXECUTION_NEXT_INDEX_COMMAND_TOPIC, currentActionIndexCommandMessage);
   }
   ...
   private void renderSequencePrimaryControlsArea()
   {
      ...
      if (sequenceStatusSubscription.getMessageNotification().poll())
      {
         ...
         if (outOfSync)
         {
            ...
            BehaviorActionSequenceTools.packActionSequenceUpdateMessage(actionSequence, actionSequenceUpdateMessage);
            ros2ControllerHelper.publish(ROS2BehaviorActionSequence.SEQUENCE_COMMAND_TOPIC, actionSequenceUpdateMessage);
         }
      }
      ...
      if (!endOfSequence && !outOfSync)
      {
         ...
         if (ImGui.checkbox(labels.get("Autonomously"), automaticExecution))
         {
            ...
            ros2ControllerHelper.publish(ROS2BehaviorActionSequence.AUTOMATIC_EXECUTION_COMMAND_TOPIC, automaticExecutionCommandMessage);
         }
         if (!canExecuteNextAction)
            ImGui.endDisabled();
         ...
         if (!automaticExecution.get())
         {
            ...
            if (canExecuteNextAction)
            {
               ...
               {
                  ros2ControllerHelper.publish(ROS2BehaviorActionSequence.MANUALLY_EXECUTE_NEXT_ACTION_TOPIC, manuallyExecuteNextActionMessage);
               }
            }
            else
            {

...
import us.ihmc.behaviors.tools.VRControlLogger;
 ...
 */
public class VRControlRecordReplay
{
   private final VRControlLogger trajectoryRecorder = new VRControlLogger("");
   ...
   public VRControlRecordReplay(ImBoolean enabledKinematicsStreaming, SideDependentList<MutableReferenceFrame> handDesiredControlFrames)
   {
   ...
   public void recordControllerData(RobotSide robotSide, boolean aButtonPressed, boolean bButtonPressed, boolean triggerPressed, float joystickX, float joystickY, Tuple3DReadOnly angularVelocity, Tuple3DReadOnly linearVelocity, ReferenceFrame recordFrame)
   {
      if (!isRecording)
         return;
   ...
   {
      return trajectoryRecorder.getJoystickX(robotSide);
   }

   public double getJoystickY(RobotSide robotSide)
   {
      return trajectoryRecorder.getJoystickY(robotSide);
   }

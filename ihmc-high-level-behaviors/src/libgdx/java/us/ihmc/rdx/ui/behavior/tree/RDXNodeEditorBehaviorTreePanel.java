...
public class RDXNodeEditorBehaviorTreePanel
{
    ...
    */
   public RDXNodeEditorBehaviorTreePanel(String name)
   {
   ...
   public void renderAsWindow(RDXBehaviorUIInterface tree)
   {
   ...
   public void renderWidgetsOnly(RDXBehaviorUIInterface tree)
   {
      NodeEditor.setCurrentEditor(context);
   ...
   private void resetNodeIndex(RDXBehaviorUIInterface tree)
   {
   ...
   public void layoutNodes(RDXBehaviorUIInterface tree)
   {
      ...
      TreeLayout<RDXBehaviorUIInterface> layout = new TreeLayout<>(treeForLayout, new NodeExtentProvider<RDXBehaviorUIInterface>()
      {
   ...
   private void renderNodeAndChildren(RDXBehaviorUIInterface node, int parentPin, ArrayList<Pair<Integer, Integer>> links)
   {
      double timeSinceLastTick = node.getTimeSinceLastTick();
      boolean isTickRecent = node.hasBeenTicked() && timeSinceLastTick < 5.0;
      ...
      for (RDXBehaviorUIInterface child : node.getUIChildren())
      {
         ImGui.sameLine();
      ...
      boolean tickedThisFrame = node.hasBeenTicked() && timeSinceLastTick < recentTickWindow;
      boolean tickedRecently = node.hasBeenTicked() && timeSinceLastTick < 1.0;

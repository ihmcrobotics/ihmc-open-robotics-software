...
public class SquareFittingFactory
{
   ...
   private void updateVertices()
   {
      ...
      for (int polygonIndex = 0; polygonIndex < planarRegion.getNumberOfConvexPolygons(); polygonIndex++)
      {
         ...
         if (polygon != null)
         {
            ...
            {
               if (DEBUG)
                  PrintTools.info("" + i + " " + polygon.getVertices()[i].getX() + " " + polygon.getVertices()[i].getY() + " "
                        + polygon.getVertices()[i].getZ());
   ...
   private void updateCenterPosition()
   {
      ...
      Point3D centerOne = new Point3D((pointOne.getX() + pointTwo.getX()) / 2, (pointOne.getY() + pointTwo.getY()) / 2,
                                      (pointOne.getZ() + pointTwo.getZ()) / 2);
      Point3D centerTwo = new Point3D((pointThree.getX() + pointFour.getX()) / 2, (pointThree.getY() + pointFour.getY()) / 2,
                                      (pointThree.getZ() + pointFour.getZ()) / 2);
      ...
      squarePosition = new Point3D((centerOne.getX() + centerTwo.getX()) / 2, (centerOne.getY() + centerTwo.getY()) / 2,
                                   (centerOne.getZ() + centerTwo.getZ()) / 2);
   ...
   private double getSingedDistancePointAndPlane(Tuple3DBasics point3d32, Point3D planeCenter, Vector3D planeNormal)
   {
      ...
      double distance = (planeA * point3d32.getX() + planeB * point3d32.getY() + planeC * point3d32.getZ() + planeD)

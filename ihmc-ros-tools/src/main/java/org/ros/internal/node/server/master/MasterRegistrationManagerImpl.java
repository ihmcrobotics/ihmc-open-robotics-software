...
public class MasterRegistrationManagerImpl
{
   ...
   public TopicRegistrationInfo registerPublisher(GraphName nodeName, URI nodeSlaveUri, GraphName topicName, String topicMessageType)
   {
      ...
      {
         LogTools.debug(String.format("Registering publisher topic %s with message type %s on node %s with slave URI %s",
                                      topicName,
                                      topicMessageType,
                                      nodeName,
                                      nodeSlaveUri));
   ...
   public boolean unregisterPublisher(GraphName nodeName, GraphName topicName)
   {
      if (LogTools.isDebugEnabled())
      {
         LogTools.debug(String.format("Unregistering publisher of topic %s from node %s", topicName, nodeName));
      }
      ...
      if (topic != null)
      {
         ...
         else
         {
            ...
            {
               LogTools.warn(String.format("Received unregister publisher for topic %s on unknown node %s", topicName, nodeName));
            }
      ...
      else
      {
         ...
         if (LogTools.isWarnEnabled())
         {
            LogTools.warn(String.format("Received unregister publisher for unknown topic %s on node %s", topicName, nodeName));
         }
   ...
   public TopicRegistrationInfo registerSubscriber(GraphName nodeName, URI nodeSlaveUri, GraphName topicName, String topicMessageType)
   {
      if (LogTools.isDebugEnabled())
      {
         LogTools.debug(String.format("Registering subscriber topic %s with message type %s on node %s with slave URI %s",
                                      topicName,
                                      topicMessageType,
                                      nodeName,
                                      nodeSlaveUri));
   ...
   public boolean unregisterSubscriber(GraphName nodeName, GraphName topicName)
   {
      if (LogTools.isDebugEnabled())
      {
         LogTools.debug(String.format("Unregistering subscriber of topic %s from node %s", topicName, nodeName));
      }
      ...
      if (topic != null)
      {
         ...
         else
         {
            ...
            if (LogTools.isWarnEnabled())
            {
               LogTools.warn(String.format("Received unregister subscriber for topic %s on unknown node %s", topicName, nodeName));
            }
            return false;
         }
      }
      else
      {
         ...
         if (LogTools.isWarnEnabled())
         {
            LogTools.warn(String.format("Received unregister subscriber for unknown topic %s on node %s", topicName, nodeName));
         }
         return false;
      }
   }
   ...
   public ServiceRegistrationInfo registerService(GraphName nodeName, URI nodeSlaveUri, GraphName serviceName, URI serviceUri)
   {
      if (LogTools.isDebugEnabled())
      {
         LogTools.debug(String.format("Registering service %s with server URI %s on node %s with slave URI %s",
                                      serviceName,
                                      serviceUri,
                                      nodeName,
                                      nodeSlaveUri));
      ...
      if (service != null)
      {
         ...
         if (previousServiceNode == node)
         {
            ...
            if (LogTools.isWarnEnabled())
            {
               LogTools.warn(String.format("Registering already known service %s with server URI %s on node %s with slave URI %s",
                                           serviceName,
                                           serviceUri,
                                           nodeName,
                                           nodeSlaveUri));
   ...
   public boolean unregisterService(GraphName nodeName, GraphName serviceName, URI serviceUri)
   {
      ...
      if (service != null)
      {
         ...
         else
         {
            ...
            if (LogTools.isWarnEnabled())
            {
               LogTools.warn(String.format("Received unregister for service %s on unknown node %s", serviceName, nodeName));
            }
      ...
      else
      {
         ...
         if (LogTools.isWarnEnabled())
         {
            LogTools.warn(String.format("Received unregister for unknown service %s on node %s", serviceName, nodeName));
         }

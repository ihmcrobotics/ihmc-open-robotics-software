...
public class DefaultPublisher<T> extends DefaultTopicParticipant implements Publisher<T>
{
   ...
   public ChannelBuffer finishHandshake(ConnectionHeader incomingHeader)
   {
      ...
      boolean messageTypeMatches = incomingType.equals(expectedType) || incomingType.equals(Subscriber.TOPIC_MESSAGE_TYPE_WILDCARD);
      ...
      boolean checksumMatches = incomingChecksum.equals(expectedChecksum) || incomingChecksum.equals(Subscriber.TOPIC_MESSAGE_TYPE_WILDCARD);
   ...
   public void addSubscriber(SubscriberIdentifier subscriberIdentifer, Channel channel)
   {
      if (DEBUG)
      {
         LogTools.info(String.format("Adding subscriber %s channel %s to publisher %s.", subscriberIdentifer, channel, this));
      }

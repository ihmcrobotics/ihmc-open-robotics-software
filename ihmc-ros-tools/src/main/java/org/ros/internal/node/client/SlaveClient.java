...
public class SlaveClient extends Client<SlaveXmlRpcEndpoint>
{
   ...
   {
      return Response.fromListChecked(xmlRpcEndpoint.paramUpdate(nodeName.toString(), name.toString(), value), new VoidResultFactory());
   }
   ...
   public Response<Void> publisherUpdate(GraphName topic, Collection<URI> publisherUris)
   {
      ...
      return Response.fromListChecked(xmlRpcEndpoint.publisherUpdate(nodeName.toString(), topic.toString(), publishers.toArray()), new VoidResultFactory());
   }

   public Response<ProtocolDescription> requestTopic(GraphName topic, Collection<String> requestedProtocols)
   {
      return Response.fromListChecked(xmlRpcEndpoint.requestTopic(nodeName.toString(), topic.toString(), new Object[][] {requestedProtocols.toArray()}),

...
public class ConnectionHeader
{
   ...
   public static ConnectionHeader decode(ChannelBuffer buffer)
   {
      ...
      while (position < readableBytes)
      {
         ...
         Preconditions.checkState(field.indexOf("=") > 0, String.format("Invalid field in handshake header: \"%s\"", field));
   ...
   public void addField(String name, String value)
   {
      ...
      {
         throw new RosRuntimeException(String.format("Unable to merge field %s: %s != %s", name, value, fields.get(name)));
      }
   }

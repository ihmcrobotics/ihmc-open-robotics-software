...
public class OutgoingMessageQueue<T>
{
   ...
   private final class Writer extends CancellableLoop
   {
      ...
      public void loop() throws InterruptedException
      {
         ...
         if (DEBUG)
         {
            LogTools.info(String.format("Writing %d bytes to %d channels.", buffer.readableBytes(), channelGroup.size()));
         }
   ...
   public void addChannel(Channel channel)
   {
      if (!writer.isRunning())
      {
         LogTools.warn("Failed to add channel. Cannot add channels after shutdown.");
         return;
      }

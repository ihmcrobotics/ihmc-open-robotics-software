...
public class RemoteUptimeClock
{
   ...
   @VisibleForTesting
   RemoteUptimeClock(LocalUptimeProvider localUptimeProvider,
                     Callable<Double> callable,
                     double driftSensitivity,
                     double errorReductionCoefficientSensitivity,
                     int latencyOutlierFilterSampleSize,
                     double latencyOutlierFilterThreshold)
   {
      ...
      Preconditions.checkArgument(errorReductionCoefficientSensitivity >= 0 && errorReductionCoefficientSensitivity <= 1);
   ...
   public void update()
   {
      ...
      {
         LogTools.warn(String.format("Measurement latency marked as outlier. Latency: %.4f s, Median: %.4f s", latency, latencyOutlierFilter.getMedian()));
         return;
      }
      ...
      if (DEBUG)
      {
         LogTools.info(String.format("localUptimeDelta: %.4g, remoteUptimeDelta: %.4g", localUptimeDelta, remoteUptimeDelta));
      }
      ...
      LogTools.info(String.format("Latency: %.4f s, Delta ratio: %.4f, Drift: %.4g, " + "Error reduction coefficient: %.4g, Error: %.4f s",
                                  latency,
                                  deltaRatio,
                                  newDrift,
                                  newErrorReductionCoefficient,
                                  error));

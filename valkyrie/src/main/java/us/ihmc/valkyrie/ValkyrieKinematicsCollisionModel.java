...
public class ValkyrieKinematicsCollisionModel implements RobotCollisionModel
{
   ...
   {
      List<Collidable> collidables = new ArrayList<>();
      SideDependentList<String> interLegCollisionNames = new SideDependentList<>("LeftInterLeg", "RightInterLeg");

      for (RobotSide robotSide : RobotSide.values)
      { // Legs
         long collisionMask = helper.getCollisionMask(interLegCollisionNames.get(robotSide));
         long collisionGroup = helper.createCollisionGroup(interLegCollisionNames.get(robotSide.getOppositeSide()));

...
PREFIX=$HOME
source $PREFIX/.val_config
source $PREFIX/.ihmc_bridge_config
scriptname=$(basename $0)
BRIDGE_NAME=valkyrie_static_bridge_yaml

# No bridge options for static bridge
BRIDGE_OPTS=$PREFIX/ihmc_bridge_ws/install/yaml_configs/valkyrie_bridged_topics.yaml
...
if [[ -x $PREFIX/exportable_bridge/run_bridge.sh ]]; then
    echo "Starting exportable bridge"
    run_bridge="$PREFIX/exportable_bridge/run_bridge.sh"
else
    echo "Starting ihmc_ws bridge"
    run_bridge="/usr/bin/python3 /opt/ros/ardent/bin/ros2 run ros1_bridge $BRIDGE_NAME $BRIDGE_OPTS"
fi

while [[ 1 ]]; do 
    $run_bridge &
    BRIDGE_PID=$!
    trap "kill $BRIDGE_PID && source $PREFIX/.val_config && rosnode kill ros_bridge; exit 0" INT TERM
    wait
done

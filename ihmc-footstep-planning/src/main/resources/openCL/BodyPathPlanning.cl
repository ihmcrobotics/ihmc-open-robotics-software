...
float *solveForPlaneCoefficients(float *covariance_matrix,
                                 float *z_variance_vector)
{
   ...
   for (int row = 0; row < 3; row++)
   {
      ...
      {
         coefficients[row] +=
             inverse_covariance_matrix[col + row * 3] * z_variance_vector[col];
      }
   }
                                            ...
                                            global float *normal_xyz_buffer)
{
   ...
   float x_coordinate = index_to_coordinate(
       idx_x, params[centerX], params[HEIGHT_MAP_RESOLUTION], center_index);
   float y_coordinate = index_to_coordinate(
       idx_y, params[centerY], params[HEIGHT_MAP_RESOLUTION], center_index);
   ...
   for (int i = 0; i < ransac_params[RANSAC_ITERATIONS]; i++)
   {
      while (true)
      {
         ...
         if (xIndex0 < 0 || xIndex0 >= cells_per_side || yIndex0 < 0 ||
             yIndex0 >= cells_per_side)
            continue;
         ...
         if (xIndex1 < 0 || xIndex1 >= cells_per_side || yIndex1 < 0 ||
             yIndex1 >= cells_per_side)
         {
            continue;
         }
         ...
         float dotProduct =
             dot(offset0, offset1) / (length(offset0) * length(offset1));
      ...
      float x_coordinate0 =
          index_to_coordinate(xIndex0, params[centerX],
                              params[HEIGHT_MAP_RESOLUTION], center_index);
      float y_coordinate0 =
          index_to_coordinate(yIndex0, params[centerY],
                              params[HEIGHT_MAP_RESOLUTION], center_index);
      ...
      float x_coordinate1 =
          index_to_coordinate(xIndex1, params[centerX],
                              params[HEIGHT_MAP_RESOLUTION], center_index);
      float y_coordinate1 =
          index_to_coordinate(yIndex1, params[centerY],
                              params[HEIGHT_MAP_RESOLUTION], center_index);
      ...
      float3 candidate_normal =
          computeNormal3DFromThreePoint3Ds(point, point0, point1);
      ...
      for (int j = 0; j < consensus_size; j++)
      {
         ...
         float x_consensus = index_to_coordinate(
             xj, params[centerX], params[HEIGHT_MAP_RESOLUTION], center_index);
         float y_consensus = index_to_coordinate(
             yj, params[centerY], params[HEIGHT_MAP_RESOLUTION], center_index);
         float z_consensus = height_map[keyConsensus];
         float3 consensus_point =
             (float3)(x_consensus, y_consensus, z_consensus);

         // check the distance of the neighboring points to the plane
         float distanceToPlane = distanceFromPoint3DToPlane3D(
             consensus_point, point, candidate_normal);

         // if the distance is less than an epsilon, we can say that point is on
         // the plane.
      ...
      if (maxConsensus >
          ransac_params[RANSAC_ACCEPTABLE_CONSENSUS] * consensusSampleSize)
      {
         break;
      }
   }
...
void kernel computeSurfaceNormalsWithLeastSquares(
    global float *params, global int *offsets, global float *height_map,
    global float *normal_xyz_buffer, global float *sampled_height)
{
   int key = get_global_id(0);
   ...
   for (int cell = 0; cell < connections; cell++)
   {
      ...
      int key_to_poll =
          indices_to_key(idx_x_to_poll, idx_y_to_poll, center_index);
   ...
   for (int cell = 0; cell < connections; cell++)
   {
      ...
      float x_coordinate_to_poll =
          index_to_coordinate(idx_x_to_poll, params[centerX],
                              params[HEIGHT_MAP_RESOLUTION], center_index);
      float y_coordinate_to_poll =
          index_to_coordinate(idx_y_to_poll, params[centerY],
                              params[HEIGHT_MAP_RESOLUTION], center_index);
   ...
   if (n > 3)
   {
      ...
      float *coefficients =
          solveForPlaneCoefficients(covariance_matrix, z_variance_vector);

      float x_solution = x / n;
      float y_solution = y / n;
      float z_solution = -coefficients[0] * x_solution -
                         coefficients[1] * y_solution - coefficients[2];
      ...
      // Given the plane equation: a*x + b*y + c*z + d = 0, with d = -(a*x0 +
      // b*y0 + c*z0), we find z:
      height_at_center = normal.s0 / normal.s2 * (x_solution - x_coordinate) +
                         normal.s1 / normal.s2 * (y_solution - y_coordinate) +
                         z_coordinate;
...
void kernel snapVertices(global float *height_map_params,
                         global float *planner_params, global float *height_map,
                         ...
                         global float *snapped_vertex_height)
{
   ...
   float2 center =
       (float2)(height_map_params[centerX], height_map_params[centerY]);
   ...
   float2 node_location = indices_to_coordinate(
       path_idx, center, path_resolution, path_center_index);

   int map_x_index = coordinate_to_index(node_location.x, center.x,
                                         map_resolution, map_center_index);
   int map_y_index = coordinate_to_index(node_location.y, center.y,
                                         map_resolution, map_center_index);
   ...
   for (int neighborIdx = 0; neighborIdx < number_of_neighbors; neighborIdx++)
   {
      ...
      int neighbor_idx_y =
          map_y_index + neighbor_offsets[1 + number_of_neighbors + neighborIdx];
      ...
      int neighbor_map_key =
          indices_to_key(neighbor_idx_x, neighbor_idx_y, map_center_index);
...
int getTraversibilityOffsetSet(int yaw_index)
{
   ...
   else if (yaw_index == 1 || yaw_index == 3 || yaw_index == 5 ||
            yaw_index == 7)
   {
      return 1;
   }
...
float computeSidedTraversibility(float *height_map_params,
                                 float *planner_params, int *offsets,
                                 int offset_set, float *height_map_data,
                                 float *normal_xyz_data, int node_side,
                                 float2 node, int neighbor_idx,
                                 float opposite_height, float nominal_height)
{
   ...
   int x_index = coordinate_to_index(node.x, height_map_params[centerX],
                                     height_map_params[HEIGHT_MAP_RESOLUTION],
                                     center_index);
   int y_index = coordinate_to_index(node.y, height_map_params[centerY],
                                     height_map_params[HEIGHT_MAP_RESOLUTION],
                                     center_index);
   ...
   float minHeight = max(opposite_height, nominal_height) -
                     planner_params[TRAVERSIBILITY_HEIGHT_WINDOW];
   float maxHeight = min(opposite_height, nominal_height) +
                     planner_params[TRAVERSIBILITY_HEIGHT_WINDOW];
   float averageHeight = 0.5f * (nominal_height + opposite_height);
   float windowWidth = (maxHeight - minHeight) / 2.0f;

   float lowestNonGroundAlpha = planner_params
       [TRAVERSIBILITY_LOWEST_NON_GROUND_DISCOUNT_WHEN_WALKING_ON_GROUND];
   float heightAboveGround =
       fabs(averageHeight - height_map_params[GROUND_HEIGHT_ESTIMATE]);
   float discountForNonGroundPointsWhenWalkingOnGround = 1.0f;
   float groundProximity = planner_params
       [TRAVERSIBILITY_HEIGHT_PROXIMITY_FOR_SAYING_WALKING_ON_GROUND];
   if (heightAboveGround < groundProximity)
   {
      discountForNonGroundPointsWhenWalkingOnGround =
          lowestNonGroundAlpha +
          (1.0f - lowestNonGroundAlpha) * heightAboveGround / groundProximity;
   }
   ...
   for (int i = 0; i < number_of_offsets; i++)
   {
      ...
      if (xQuery < 0 || yQuery < 0 || xQuery >= cells_per_side ||
          yQuery >= cells_per_side)
         continue;
      ...
      if (heightQuery > minHeight && heightQuery < maxHeight)
      {
         ...
         float deltaHeight =
             max(0.0f, fabs(averageHeight - heightQuery) -
                           planner_params[TRAVERSIBILITY_HEIGHT_DEADBAND]);
         float cellPercentage = 1.0f - deltaHeight / windowWidth;
         float nonGroundDiscount = 1.0f;

         if (!epsilonEquals(heightQuery,
                            height_map_params[GROUND_HEIGHT_ESTIMATE], 1e-3))
         {
         ...
         float incline = max(
             0.0f, acos(query_normal_z) -
                       planner_params[TRAVERSIBILITY_MIN_NORMAL_TO_PENALIZE]);
         float inclineAlpha =
             (planner_params[TRAVERSIBILITY_MAX_NORMAL_TO_PENALIZE] - incline) /
             (planner_params[TRAVERSIBILITY_MAX_NORMAL_TO_PENALIZE] -
              planner_params[TRAVERSIBILITY_MIN_NORMAL_TO_PENALIZE]);
         inclineAlpha = clamp(inclineAlpha, 0.0f, 1.0f);
         traversibilityScoreNumber +=
             nonGroundDiscount *
             ((1.0f - planner_params[TRAVERSIBILITY_INCLINE_WEIGHT]) *
                  cellPercentage +
...
float4 computeTraversibilityMeasures(
    global float *height_map_params, global float *planner_params,
    global int *traversibility_offsets, global float *snapped_vertex_height,
    global float *height_map_data, global float *normal_xyz_data,
    float2 parent_node, float2 child_node, int yaw_idx)
{
   ...
   float2 grid_center =
       (float2)(height_map_params[centerX], height_map_params[centerY]);
   int parent_key =
       coordinate_to_key(parent_node, grid_center,
                         planner_params[PATH_RESOLUTION], path_center_index);
   int child_key =
       coordinate_to_key(child_node, grid_center,
                         planner_params[PATH_RESOLUTION], path_center_index);
   int parent_planner_x =
       (int)round(parent_node.x / planner_params[PATH_RESOLUTION]);
   int parent_planner_y =
       (int)round(parent_node.y / planner_params[PATH_RESOLUTION]);
   ...
   float left_traversibility = computeSidedTraversibility(
       height_map_params, planner_params, traversibility_offsets, offset_set,
       height_map_data, normal_xyz_data, 0, child_node, yaw_idx, parent_height,
       child_height);
   float right_traversibility = computeSidedTraversibility(
       height_map_params, planner_params, traversibility_offsets, offset_set,
       height_map_data, normal_xyz_data, 1, child_node, yaw_idx, parent_height,
       child_height);

   float previous_left_traversibility = 1.0f;
   float previous_right_traversibility = 1.0f;
   if (parent_planner_x != planner_params[START_X_INDEX] &&
       parent_planner_y != planner_params[START_Y_INDEX])
   {
      previous_left_traversibility = computeSidedTraversibility(
          height_map_params, planner_params, traversibility_offsets, offset_set,
          height_map_data, normal_xyz_data, 0, parent_node, yaw_idx,
          child_height, parent_height);
      previous_right_traversibility = computeSidedTraversibility(
          height_map_params, planner_params, traversibility_offsets, offset_set,
          height_map_data, normal_xyz_data, 1, parent_node, yaw_idx,
          child_height, parent_height);
   }

   float leftStepScore =
       sqrt(left_traversibility * previous_right_traversibility);
   float rightStepScore =
       sqrt(right_traversibility * previous_left_traversibility);

   return (float4)(left_traversibility, right_traversibility, leftStepScore,
                   rightStepScore);
}

float computeTraversibilityCost(global float *planner_params,
                                float4 traversibility_measures)
{
   float stanceTraversibility =
       max(traversibility_measures.s0, traversibility_measures.s1);
   float stepTraversibility =
       max(traversibility_measures.s2, traversibility_measures.s3);

   return planner_params[TRAVERSIBILITY_WEIGHT] *
          (planner_params[TRAVERSIBILITY_STANCE_ALPHA] *
               (1.0f - stanceTraversibility) +
           planner_params[TRAVERSIBILITY_STEP_ALPHA] *
               (1.0f - stepTraversibility));
}

bool computeIsTraversible(global float *planner_params,
                          float4 traversibility_measures)
{
   return traversibility_measures.s0 >
              planner_params[MIN_TRAVERSIBILITY_PERCENTAGE] ||
          traversibility_measures.s1 >
              planner_params[MIN_TRAVERSIBILITY_PERCENTAGE];
}

float computeRollAtNode(global float *height_map_params,
                        global float *normal_least_squares_xyz_buffer,
                        float2 parent, float2 neighbor)
{
   float2 center =
       (float2)(height_map_params[centerX], height_map_params[centerY]);
   ...
   int body_map_key = coordinate_to_key(averageBodyPosition, center,
                                        map_resolution, map_center_index);
...
{
   float maxPenalizedRoll =
       planner_params[MAX_PENALIZED_ROLL_ANGLE] - planner_params[ROLL_DEADBAND];
   float inclineScale = clamp(fabs(incline) / maxPenalizedRoll, 0.0f, 1.0f);
   float deadbandedRollAngle =
       max(0.0f, fabs(roll) - planner_params[ROLL_DEADBAND]);
...
bool collisionDetected(global float *height_map_params,
                       ...
                       global int *collision_offsets, global float *height_map,
                       float2 point, int yaw_index, float height)
{
   int map_center_index = height_map_params[CENTER_INDEX];
   float map_resolution = height_map_params[HEIGHT_MAP_RESOLUTION];
   float2 center =
       (float2)(height_map_params[centerX], height_map_params[centerY]);
   int map_idx_x =
       coordinate_to_index(point.x, center.x, map_resolution, map_center_index);
   int map_idx_y =
       coordinate_to_index(point.y, center.y, map_resolution, map_center_index);
   ...
   for (int i = 0; i < number_of_offsets; i++)
   {
      ...
      int xQuery =
          map_idx_x + computeCollisionOffsetX(yaw_index, x_offset, y_offset);
      int yQuery =
          map_idx_y + computeCollisionOffsetY(yaw_index, x_offset, y_offset);
                                 ...
                                 global float *heuristic_cost_map)
{
   ...
   float2 center =
       (float2)(height_map_params[centerX], height_map_params[centerY]);
   float2 node_position = indices_to_coordinate(
       node_index, center, path_resolution, path_center_index);
   float2 goal_position =
       (float2)(planner_params[GOAL_X], planner_params[GOAL_Y]);

   heuristic_cost_map[path_key] = length(goal_position - node_position);
}

void kernel computeEdgeData(
    global float *height_map_params, global float *planner_params,
    global int *neighbor_offsets, global int *traversibility_offsets,
    global int *collision_offsets, global float *height_map,
    global float *snapped_height_map,
    global float *normal_least_squares_xyz_buffer,
    global float *normal_ransac_xyz_buffer, global int *edge_rejection_reason,
    global float *delta_height_map, global float *incline_map,
    global float *roll_map, global float *stance_traversibility_map,
    global float *step_traversibility_map, global float *incline_cost_map,
    global float *roll_cost_map, global float *traversibility_cost_map,
    global float *edge_cost_map)
{
   ...
   {
      // for (int neighborIdx = 0; neighborIdx < number_of_neighbors;
      // neighborIdx++)
      // {
   ...
   int neighbor_idx_y =
       idx_y + neighbor_offsets[1 + number_of_neighbors + neighborIdx];
   int edge_key = number_of_neighbors * path_key + neighborIdx;
   ...
   float2 neighbor_position = indices_to_coordinate(
       neighbor_index, center, path_resolution, path_center_index);
   int neighbor_path_key =
       indices_to_key(neighbor_idx_x, neighbor_idx_y, path_center_index);
   ...
   {
      if (collisionDetected(height_map_params, planner_params,
                            collision_offsets, height_map, neighbor_position,
                            neighborIdx, snapped_neighbor_height))
      {
   ...
   float4 traversibility_measures = computeTraversibilityMeasures(
       height_map_params, planner_params, traversibility_offsets,
       snapped_height_map, height_map, normal_ransac_xyz_buffer, node_position,
       neighbor_position, neighborIdx);
   ...
   if (incline > planner_params[NOMINAL_INCLINE])
   {
      ...
      incline_cost =
          planner_params[INCLINE_COST_WEIGHT] *
          (max(0.0f, inclineDelta - planner_params[INCLINE_COST_DEADBAND]));
   }
   ...
   if (planner_params[COMPUTE_SURFACE_NORMALS] == 1.0f)
   {
      // compute roll cost
      float roll =
          computeRollAtNode(height_map_params, normal_least_squares_xyz_buffer,
                            node_position, neighbor_position);
   ...
   if (planner_params[COMPUTE_SURFACE_NORMALS] == 1.0f &&
       planner_params[COMPUTE_TRAVERSIBILITY])
   {
      // get the traversibility cost
      float traversibility_cost =
          computeTraversibilityCost(planner_params, traversibility_measures);
...
float computeSmootherTraversibility(
    global float *height_map_params, global float *planner_params,
    global float *smoothing_params, global float *traversibility_offsets,
    global float *height_map_data, global float *normal_xyz_data, int side,
    float signY, float nominal_height, float2 waypoint_position,
    float waypoint_yaw)
{
   ...
   float min_height =
       nominal_height - planner_params[TRAVERSIBILITY_HEIGHT_WINDOW];
   float max_height =
       nominal_height + planner_params[TRAVERSIBILITY_HEIGHT_WINDOW];
   ...
   for (int i = 0; i < number_of_offsets; i++)
   {
      ...
      int twiddled_x_index = coordinate_to_index(twiddled_position.x, center.x,
                                                 resolution, center_index);
      int twiddled_y_index = coordinate_to_index(twiddled_position.y, center.y,
                                                 resolution, center_index);

      if (twiddled_x_index < 0 || twiddled_y_index < 0 ||
          twiddled_x_index >= cells_per_side ||
          twiddled_y_index >= cells_per_side)
         continue;

      int twiddled_key =
          indices_to_key(twiddled_x_index, twiddled_y_index, center_index);
      ...
      {
         if (epsilonEquals(twiddled_height,
                           height_map_params[GROUND_HEIGHT_ESTIMATE], 1e-3))
         {
         ...
         else
         {
            ...
            if (twiddled_height - height_map_params[GROUND_HEIGHT_ESTIMATE] <
                planner_params[TRAVERSIBILITY_HEIGHT_WINDOW])
            {
            ...
            float height_deadband =
                smoothing_params[TRAVERSIBILITY_SMOOTHING_HEIGHT_DEADBAND];
            float delta_height = max(
                0.0f, fabs(nominal_height - twiddled_height) - height_deadband);
            float cell_percentage =
                1.0f -
                delta_height / planner_params[TRAVERSIBILITY_HEIGHT_WINDOW];
            ...
            traversibility_score_numerator +=
                cell_percentage *
                ((1.0f - planner_params[TRAVERSIBILITY_INCLINE_WEIGHT]) *
                     non_ground_alpha +
...
float computeDeltaHeadingMagnitude(float x0, float y0, float x1, float y1,
                                   float x2, float y2, float deadband)
{
   float heading0 = atan2(y1 - y0, x1 - x0);
   float heading1 = atan2(y2 - y1, x2 - x1);
   return max(fabs(angleDifferenceMinusPiToPi(heading1, heading0)) - deadband,
              0.0f);
}

void kernel computeCurrentTraversibilityMap(
    global float *height_map_params, global float *planner_params,
    global float *smoother_params, global float *traversibility_nominal_offsets,
    global float *height_map_data, global float *snapped_node_height_data,
    global float *normal_xyz_data, global float *left_traversibility_map,
    global float *right_traversibility_map)
{
   ...
   float2 waypoint_position = indices_to_coordinate(
       (int2)(idx_x, idx_y), center, resolution, center_index);
   int path_key = coordinate_to_key(waypoint_position, center,
                                    planner_params[PATH_RESOLUTION],
                                    planner_params[PATH_CENTER_INDEX]);
   float waypoint_z = snapped_node_height_data[path_key];
   float waypoint_yaw =
       index_to_yaw(yaw_key, smoother_params[YAW_DISCRETIZATIONS]);

   float left_traversibility = computeSmootherTraversibility(
       height_map_params, planner_params, smoother_params,
       traversibility_nominal_offsets, height_map_data, normal_xyz_data, 0,
       1.0f, waypoint_z, waypoint_position, waypoint_yaw);
   float right_traversibility = computeSmootherTraversibility(
       height_map_params, planner_params, smoother_params,
       traversibility_nominal_offsets, height_map_data, normal_xyz_data, 1,
       1.0f, waypoint_z, waypoint_position, waypoint_yaw);
                                        ...
                                        global float *max_collision_map)
{
   ...
   int maxOffset =
       (int)round(0.5 * length((float2)(box_size_x, box_size_y)) / resolution);
   int center_index = height_map_params[CENTER_INDEX];

   float2 center =
       (float2)(height_map_params[centerX], height_map_params[centerY]);
   float2 waypoint_xy = indices_to_coordinate((int2)(idx_x, idx_y), center,
                                              resolution, center_index);
   float heading = index_to_yaw(yaw_key, smoothing_params[YAW_DISCRETIZATIONS]);

   int path_key =
       coordinate_to_key(waypoint_xy, center, planner_params[PATH_RESOLUTION],
                         planner_params[PATH_CENTER_INDEX]);
   ...
   for (int xi = -maxOffset; xi <= maxOffset; xi++)
   {
      for (int yi = -maxOffset; yi <= maxOffset; yi++)
      {
         ...
         if (indexXi < 0 || indexXi >= cells_per_axis || indexYi < 0 ||
             indexYi >= cells_per_axis)
            continue;

         int2 indices = (int2)(indexXi, indexYi);
         float2 position =
             indices_to_coordinate(indices, center, resolution, center_index);

         float2 delta = position - waypoint_xy;

         // rotating this offset to the local frame
         // float2 delta_local = (float2) (cH * delta.x + sH * delta.y, -sH *
         // delta.x + cH * delta.y);
         float2 delta_local = apply2DRotationToVector2D(delta, cH, sH);

         float absDyLocal = fabs(delta_local.y);

         if (fabs(delta_local.x) > half_box_size_x ||
             absDyLocal > half_box_size_y)
            continue;
         ...
         float2 penetration =
             (float2)(0.0f, sign(delta_local.y) * lateral_penetration);
...
void kernel computeTraversibilityForGradientMap(
    global float *height_map_params, global float *planner_params,
    ...
    global float *right_traversibility_for_gradient_map)
{
   ...
   float left_neg_traversibility = computeSmootherTraversibility(
       height_map_params, planner_params, smoother_params,
       traversibility_gradient_offsets, height_map_data, normal_xyz_data, 0,
       -1.0f, waypoint_z, waypoint_position, waypoint_yaw);
   float left_pos_traversibility = computeSmootherTraversibility(
       height_map_params, planner_params, smoother_params,
       traversibility_gradient_offsets, height_map_data, normal_xyz_data, 0,
       1.0f, waypoint_z, waypoint_position, waypoint_yaw);
   float right_neg_traversibility = computeSmootherTraversibility(
       height_map_params, planner_params, smoother_params,
       traversibility_gradient_offsets, height_map_data, normal_xyz_data, 1,
       -1.0f, waypoint_z, waypoint_position, waypoint_yaw);
   float right_pos_traversibility = computeSmootherTraversibility(
       height_map_params, planner_params, smoother_params,
       traversibility_gradient_offsets, height_map_data, normal_xyz_data, 1,
       1.0f, waypoint_z, waypoint_position, waypoint_yaw);
   ...
   int map_key = indices_to_key(idx_x, idx_y, center_index);
   int result_key = smoother_params[YAW_DISCRETIZATIONS] * map_key + yaw_key;
   left_traversibility_for_gradient_map[2 * result_key] =
       left_neg_traversibility;
   left_traversibility_for_gradient_map[2 * result_key + 1] =
       left_pos_traversibility;
   right_traversibility_for_gradient_map[2 * result_key] =
       right_neg_traversibility;
   right_traversibility_for_gradient_map[2 * result_key + 1] =
       right_pos_traversibility;
}

void kernel computeGroundPlaneGradientMap(
    global float *height_map_params, global float *planner_params,
    global float *smoother_params, global int *ground_plane_offsets,
    global float *height_map_data, global int *left_cells_map,
    global int *right_cells_map, global float *gradient_map)
{
   ...
   for (int side = 0; side < 2; side++)
   {
      ...
      for (int i = 0; i < offsets; i++)
      {
         ...
         int query_idx_x = coordinate_to_index(
             query_position.x, center.x,
             height_map_params[HEIGHT_MAP_RESOLUTION], center_index);
         int query_idx_y = coordinate_to_index(
             query_position.y, center.y,
             height_map_params[HEIGHT_MAP_RESOLUTION], center_index);

         if (query_idx_x < 0 || query_idx_x >= cells_per_side ||
             query_idx_y < 0 || query_idx_y >= cells_per_side)
            continue;
         ...
         bool isGroundPlane =
             fabs(query_z - height_map_params[GROUND_HEIGHT_ESTIMATE]) < 1e-3;
         if (isGroundPlane)
            ground_plane_cells++;
      }
   ...
   float ground_plane_cell_count_delta =
       ((float)(left_cells - right_cells)) / offsets;
   float2 gradient_basis = (float2)(sH, cH);
   float2 gradient = ground_plane_cell_count_delta *
                     smoother_params[FLAT_GROUND_WEIGHT] * gradient_basis;

   gradient_map[2 * results_key] = gradient.x;
   gradient_map[2 * results_key + 1] = gradient.y;
}

void kernel computeWaypointSmoothnessGradient(
    global float *smoothing_params, global float *waypoint_xyzYaw,
    ...
    global float *waypoint_smoothness_gradients)
{
   ...
   if (waypoint_key == 0 ||
       waypoint_key == (smoothing_params[NUMBER_OF_WAYPOINTS] - 1))
      return;
   ...
   float spacingGradientX =
       -4.0f * smoothing_params[EQUAL_SPACING_WEIGHT] * (x2 - 2.0f * x1 + x0);
   float spacingGradientY =
       -4.0f * smoothing_params[EQUAL_SPACING_WEIGHT] * (y2 - 2.0f * y1 + y0);
   float alphaTurnPoint =
       isTurnPoint ? smoothing_params[TURN_POINT_SMOOTHNESS_DISCOUNT] : 1.0f;

   float2 gradient =
       alphaTurnPoint * (float2)(spacingGradientX, spacingGradientY);

   // Smoothness gradient
   if (!isTurnPoint)
   {
      float min_curvature_to_penalize =
          smoothing_params[MIN_CURVATURE_TO_PENALIZE];
      float gradient_epsilon = smoothing_params[GRADIENT_EPSILON];
      float smoothness_weight = smoothing_params[SMOOTHNESS_WEIGHT];
      float exp = 1.5f;
      double f0 = pow(computeDeltaHeadingMagnitude(x0, y0, x1, y1, x2, y2,
                                                   min_curvature_to_penalize),
                      exp);
      double fPdx =
          pow(computeDeltaHeadingMagnitude(x0, y0, x1 + gradient_epsilon, y1,
                                           x2, y2, min_curvature_to_penalize),
              exp);
      double fPdy =
          pow(computeDeltaHeadingMagnitude(x0, y0, x1, y1 + gradient_epsilon,
                                           x2, y2, min_curvature_to_penalize),
              exp);
      float smoothnessGradientX =
          smoothness_weight * (fPdx - f0) / gradient_epsilon;
      float smoothnessGradientY =
          smoothness_weight * (fPdy - f0) / gradient_epsilon;
...
void kernel getWaypointCurrentTraversibility(
    global float *height_map_params, global float *smoothing_params,
    global float *waypoint_xyzYaw, global float *left_traversibility_map,
    global float *right_traversibility_map,
    global float *waypoint_traversibility_values)
{
   ...
   float2 waypoint_xy = (float2)(waypoint_xyzYaw[4 * waypoint_key],
                                 waypoint_xyzYaw[4 * waypoint_key + 1]);
   float2 center =
       (float2)(height_map_params[centerX], height_map_params[centerY]);
   int center_index = height_map_params[CENTER_INDEX];

   int map_key = coordinate_to_key(waypoint_xy, center,
                                   height_map_params[HEIGHT_MAP_RESOLUTION],
                                   center_index);
   int yaw_key = yaw_to_index(waypoint_xyzYaw[4 * waypoint_key + 3],
                              smoothing_params[YAW_DISCRETIZATIONS]);

   int data_key = smoothing_params[YAW_DISCRETIZATIONS] * map_key + yaw_key;

   waypoint_traversibility_values[2 * waypoint_key] =
       left_traversibility_map[data_key];
   waypoint_traversibility_values[2 * waypoint_key + 1] =
       right_traversibility_map[data_key];
}

void kernel computeWaypointMapGradients(
    global float *height_map_params, global float *smoothing_params,
    global float *waypoint_xyzYaw, global int *waypont_turn_points,
    global float *collision_gradient_map, global int *max_collisions_map,
    ...
    global float *waypoint_ground_plane_gradients)
{
   ...
   // return if it's the first or last gradient
   if (waypoint_key < 2 ||
       waypoint_key > (smoothing_params[NUMBER_OF_WAYPOINTS] - 3))
      return;
   ...
   waypoint_collision_gradients[2 * waypoint_key] =
       collision_gradient_map[2 * data_key];
   waypoint_collision_gradients[2 * waypoint_key + 1] =
       collision_gradient_map[2 * data_key + 1];

   // get the traversibility gradient
   for (int side = 0; side <= 1; side++)
   {
      float local_traversibility_threshold =
          smoothing_params[SMOOTHER_TRAVERSIBILITY_THRESHOLD];

      float current_traversibility =
          waypoint_traversibility_values[2 * waypoint_key + side];
      float previous_traversibility_0 =
          waypoint_traversibility_values[2 * (waypoint_key - 1) + side];
      float previous_traversibility_1 =
          waypoint_traversibility_values[2 * (waypoint_key - 2) + side];
      float next_traversibility_0 =
          waypoint_traversibility_values[2 * (waypoint_key + 1) + side];
      float next_traversibility_1 =
          waypoint_traversibility_values[2 * (waypoint_key + 2) + side];
      float max_local_traversibility =
          max(max(max(max(current_traversibility, previous_traversibility_0),
                      previous_traversibility_1),
                  next_traversibility_0),
              next_traversibility_1);
      ...
      else
      {
         ...
         float discount =
             (local_traversibility_threshold - max_local_traversibility) /
             (local_traversibility_threshold -
              smoothing_params[TRAVERSIBILITY_THRESHOLD_FOR_NO_DISCOUNT]);
         discount = clamp(discount, 0.0f, 1.0f);

         float2 delta_local =
             (float2)(0.0f, discount * (positive_sample - negative_sample));
         float2 gradient = applyInverseYawRotationToVector2D(
             delta_local, waypoint_xyzYaw[4 * waypoint_key + 3]);
         ;
   ...
   float next_height_above_ground_plane =
       waypoint_xyzYaw[4 * (waypoint_key + 1) + 3] - ground_plane_estimate;

   if (waypont_turn_points[waypoint_key] == 1 ||
       current_height_above_ground_plane > height_threshold_for_ground ||
   ...
   {
      waypoint_ground_plane_cells[2 * waypoint_key] =
          left_ground_plane_cells_map[data_key];
      waypoint_ground_plane_cells[2 * waypoint_key + 1] =
          right_ground_plane_cells_map[data_key];
      waypoint_ground_plane_gradients[2 * waypoint_key] =
          ground_plane_gradient_map[2 * data_key];
      waypoint_ground_plane_gradients[2 * waypoint_key + 1] =
          ground_plane_gradient_map[2 * data_key + 1];
   }
}
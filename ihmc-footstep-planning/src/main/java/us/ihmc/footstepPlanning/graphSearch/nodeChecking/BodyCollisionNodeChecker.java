...
public class BodyCollisionNodeChecker extends FootstepNodeChecker
{
   ...
   {
      int hashcode = node.getPlanarRegionsHashCode() * 31 + previousNode.getPlanarRegionsHashCode();
      if (midpointMap.containsKey(hashcode))
         return midpointMap.get(hashcode);

      List<PlanarRegion> nodePlanes = snapper.getOrCreateNearbyRegions(node.getRoundedX(), node.getRoundedY());
   ...
   {
      FramePoint3D midPoint = getMidPoint(node, previousNode);
      if (midPoint == null)
         return false;

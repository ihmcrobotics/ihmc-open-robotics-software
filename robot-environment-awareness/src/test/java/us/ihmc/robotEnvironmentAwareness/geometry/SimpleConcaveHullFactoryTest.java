...
public class SimpleConcaveHullFactoryTest
{
   ...
   public void testSimplePointcloudFormingASquare()
   {
      ...
      ConcaveHullCollection concaveHullCollection = SimpleConcaveHullFactory.createConcaveHullCollection(data.getPointCloudInPlane(), data.getIntersections(),
                                                                                                         parameters);
   ...
   public void testOverlappingLineConstraints()
   {
      ...
      List<LineSegment2D> polygon2 = polygon1.stream().map(LineSegment2D::new).peek(segment -> segment.translate(0.09, 0.005)).collect(Collectors.toList());

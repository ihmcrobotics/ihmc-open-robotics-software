...
public class BWCPlanarWalkingController implements Controller
{
   ...
   private final YoDouble desirdLegLength = new YoDouble("desiredLegLength", registry);
   ...
   public BWCPlanarWalkingController(BWCPlanarWalkingRobot controllerRobot)
   {
      ...
      desirdLegLength.set((BWCPlanarWalkingRobotDefinition.shinLength + BWCPlanarWalkingRobotDefinition.thighLength) / 2.0);
   ...
   public void doControl()
   {
      ...
      for (RobotSide robotSide : RobotSide.values)
      {
         ...
         double desiredLegLength = desirdLegLength.getDoubleValue();
         double desiredKneeVelocity = 0.0;
         // compute and record the desired torque to hold the leg at the desired length
         double torque = kneeLengthControllers.get(robotSide).compute(legLength, desiredLegLength, kneeVelocity, desiredKneeVelocity);

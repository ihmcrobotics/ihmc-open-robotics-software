...
public class CentroidalDynamicsRobot implements FullRobotModelFactory {
	...
	private void createSCSRobot(YoGraphicsListRegistry graphicsListRegistry) {
		...
		scsRobot.setController(new CentroidalRobotController(forcePoint, tempVector, controllerFullRobotModel,
				scsRobot.getYoTime(), graphicsListRegistry));
	...
	public FullRobotModel createFullRobotModel() {
		...
		FullRobotModelFromDescription fullRobotModel = new FullRobotModelFromDescription(robotDescription,
				getJointMap(), null);
	...
	public class CentroidalRobotController implements RobotController {
		...
		public CentroidalRobotController(ExternalForcePoint forcePoint, Vector3D gravity,
				FullRobotModel controllerFullRobotModel, YoDouble yoTime, YoGraphicsListRegistry graphicsListRegistry) {
			...
			this.stateMachine = new GenericStateMachine<>(namePrefix + "State", namePrefix + "TimeInState",
					CentroidalRobotStateEnum.class, yoTime, registry);
		...
		private void createExternalForcePointGraphic(YoGraphicsListRegistry graphicsListRegistry,
				ExternalForcePoint externalForcePoint) {
			YoFrameVector3D yoForceVector = externalForcePoint.getYoForce();
			YoFramePoint3D yoForcePoint = externalForcePoint.getYoPosition();
			YoFramePoint3D yoGroundPoint = new YoFramePoint3D(robotName + "ProjectedFocePoint",
					yoForcePoint.getReferenceFrame(), registry);
			...
			YoGraphicVector forceVisualization = new YoGraphicVector(robotName + "ForceVisualization", yoGroundPoint,
					yoForceVector, 0.005, new YoAppearanceRGBColor(Color.RED, 0.0), true);
	...
	public abstract class CentroidalRobotState extends State<CentroidalRobotStateEnum> {
		...
		public CentroidalRobotState(CentroidalRobotStateEnum stateEnum, ExternalForcePoint forcePoint, Vector3D gravity,
				FullRobotModel robotModel, CentroidalMotionPlanner motionPlanner) {
			super(stateEnum);
			this.forcePoint = forcePoint;
			this.gravity = gravity;
	...
	public class GroundState extends CentroidalRobotState {
		...
		private RecyclingArrayList<CentroidalMotionNode> motionPlannerNode = new RecyclingArrayList<>(
				CentroidalMotionNode.class);

		public GroundState(ExternalForcePoint forcePoint, Vector3D gravity, FullRobotModel robotModel,
				CentroidalMotionPlanner motionPlanner, YoVariableRegistry registry) {
		...
		public void planMotion() {
			...
			finalPositionConstraintType.set(DependentVariableConstraintType.IGNORE,
					DependentVariableConstraintType.EQUALITY, DependentVariableConstraintType.EQUALITY);

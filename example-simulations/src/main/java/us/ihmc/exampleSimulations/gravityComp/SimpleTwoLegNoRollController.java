...
public class SimpleTwoLegNoRollController implements RobotController
{
   private static final ReferenceFrame worldFrame = ReferenceFrame.getWorldFrame();

   private final YoRegistry registry = new YoRegistry(getClass().getSimpleName());
   private final GravityCoriolisExternalWrenchMatrixCalculator gravityCompensator;
   private final SimpleTwoLeggedNoRollRobot robot;
   private final CenterOfMassReferenceFrame centerOfMassFrame;

   private final double robotMass;

   public SimpleTwoLegNoRollController(double controlDT, SimpleTwoLeggedNoRollRobot robot, YoGraphicsListRegistry yoGraphicsListRegistry)
   {
      this.robot = robot;
